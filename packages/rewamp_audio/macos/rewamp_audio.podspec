Pod::Spec.new do |s|
  ...
  if ENV['REWAMP_WITH_XMP'] != '0'
    preprocessor << 'REWAMP_WITH_XMP=1'   # registry + subsong dispatch, pod-wide
...
#define LIBXMP_NO_PROWIZARD 1'
  ...
  if ENV['REWAMP_WITH_FURNACE'] != '0'
    ...
    # binaire n'avait zlib que par les blocs VGM/GME/… — une dépendance par
    # accident, qui disparaissait avec eux.
    libs << 'z' unless libs.include?('z')
...
#define _DARWIN_C_SOURCE 1'
  ...
  if ENV['REWAMP_WITH_KSS'] != '0'
    ...
    preprocessor << 'MUS2KSS_LIBRARY=1'
  ...
  if ENV['REWAMP_WITH_MT32'] != '0'
    preprocessor << 'REWAMP_WITH_MT32=1'
    preprocessor << 'MT32EMU_WITH_INTERNAL_RESAMPLER=1'
    preprocessor << 'MT32EMU_WITH_STD_SNPRINTF=1'
    header_dirs  << '$(PODS_TARGET_SRCROOT)/../third_party/mt32emu'
    header_dirs  << '$(PODS_TARGET_SRCROOT)/../third_party/tml'
