Pod::Spec.new do |s|
  ...
  # iOS 13 minimum: the floor ANGLE is built against
  # (REWAMP_IOS_MIN in scripts/build_angle_ios.sh — keep the two in sync).
  s.platform = :ios, '15.0'
  ...
  # --- libopenmpt ------------------------------------------------------------
  # Enable with: REWAMP_WITH_OPENMPT=1 flutter run -d iphone
  # Build the XCFramework first: scripts/build_libopenmpt_ios.sh
  if ENV['REWAMP_WITH_OPENMPT'] != '0'
    vendored_frameworks << 'Libs/libopenmpt.xcframework'
    libraries  << 'c++'
    defines    << 'REWAMP_WITH_OPENMPT=1'
...
#define LIBXMP_NO_PROWIZARD 1'
  ...
  # source, same pinned commit and same patch as the macOS dylibs, so both Apple
  # platforms now run ONE GL library. That patch is what brings
  # GL_EXT_shader_framebuffer_fetch (projectM's CUSTOMSHAPE_FAST_RENDER) to iOS;
  # the MetalANGLE fork this replaced never had it.
  # Static, not a framework: nothing to embed or code-sign, and libEGL_static
  # links libGLESv2_static directly (no ModuleDir dlopen as on macOS).
  # Unconditional, like the macOS podspec: the visualizers are not optional. It
  # used to hang off REWAMP_WITH_METALANGLE=1, the pod's only strict opt-in flag,
  # which a fork reusing the pod without app/ios/Podfile silently lost.
  vendored_frameworks << 'Libs/angle/ANGLE.xcframework'
  frameworks += ['Metal', 'IOSurface', 'QuartzCore', 'CoreVideo', 'CoreGraphics']
  libraries  << 'c++' unless libraries.include?('c++')
  defines    << 'REWAMP_WITH_ANGLE=1'
  defines    << 'REWAMP_GL_GLES=1'
  # Static ANGLE: entry points are plain symbols, not visibility-annotated
  # imports (mirrors BUILD.gn's angle_static_config).
  defines    += ['ANGLE_STATIC=1', 'EGLAPI=', 'GL_APICALL=', 'GL_API=']
  search_paths << '$(PODS_TARGET_SRCROOT)/Libs/angle/include'

  prepare_parts << <<-'SH'
      CLASSES="$(pwd)/Classes"
      {
        printf '#ifdef REWAMP_GL_GLES\n'
  ...
  # --- libvgm ----------------------------------------------------------------
  # Enabled by default; opt out with REWAMP_WITH_VGM=0.
  # (or set in app/ios/Podfile: ENV['REWAMP_WITH_VGM'] = '1')
  #
  # Same strategy as the macOS podspec: prepare_command generates one wrapper
  # per libvgm source inside Classes/vgm_cores/ so each becomes its own
  # Xcode translation unit (required — cores share file-static names).
  if ENV['REWAMP_WITH_VGM'] != '0'
    libvgm_root = '$(PODS_TARGET_SRCROOT)/../third_party/libvgm/libvgm'

    libraries  << 'c++' unless libraries.include?('c++')
    libraries  << 'iconv'   # StrUtils-CPConv_IConv (system libiconv on iOS)
    libraries  << 'z'       # FileLoader .vgz gzip via zlib

    defines    << 'REWAMP_WITH_VGM=1'
    defines    += %w[
    ...
    # Same prepare_command as macOS: paths are identical because both ios/ and
    # macos/ sit at the same depth relative to the rewamp_audio package root.
    prepare_parts << <<-'SH'
      ...
      # Chip cores — skip include-only helpers:
  ...
  if ENV['REWAMP_WITH_GME'] != '0'
    ...
    defines    << 'REWAMP_WITH_ARCHIVE=1'
    defines    << 'HAVE_CONFIG_H=1'                # archive_platform.h + sysdefs.h
    defines    << 'TUKLIB_SYMBOL_PREFIX=lzma_'     # avoid symbol conflicts (same as Modizer)
    search_paths << libarchive_root                            # for archive.h / config.h
    search_paths << liblzma_root                               # for liblzma config.h
    search_paths << "#{liblzma_root}/src/liblzma/api"         # lzma.h (public API)
    # liblzma internal subdirs (cross-subdir includes require explicit paths)
    %w[common check lz lzma delta simple rangecoder api].each do |d|
      search_paths << "#{liblzma_root}/src/liblzma/#{d}"
    end
    search_paths << "#{liblzma_root}/src/common"              # tuklib headers
    ...
    # unrar: official UnRAR library — handles *solid* RARv3/v4 archives that
    # libarchive cannot.  RSN files (SPC sets) are solid RARs; libgme's Rsn_Emu
    # uses this via RARDLL + RAR_HDR_DLL_HPP.  RAR_SMP is left undefined so unrar
    # stays single-threaded (no pthread; threadpool.cpp compiles to nothing).
    unrar_root = '$(PODS_TARGET_SRCROOT)/../third_party/unrar'
    defines    << 'REWAMP_WITH_UNRAR=1'
    defines    << 'RARDLL=1'
    defines    << 'RAR_HDR_DLL_HPP=1'      # libgme Spc_Emu.cpp → #include <dll.hpp>
    defines    << 'SILENT=1'
    defines    << '_FILE_OFFSET_BITS=64'   # no-op on Apple (off_t already 64-bit)
    defines    << '_LARGEFILE_SOURCE=1'
    search_paths << unrar_root

    # Only the .cpp files UnRAR's makefile compiles (OBJECTS + LIB_OBJ); the rest
    # are #included by those or unused, and break the build if compiled standalone.
    prepare_parts << <<-'SH'
  ...
  # --- libgme ----------------------------------------------------------------
  # Enabled by default; opt out with REWAMP_WITH_GME=0.
  ...
  if ENV['REWAMP_WITH_GME'] != '0'
    ...
    libraries  << 'c++' unless libraries.include?('c++')
    libraries  << 'z'      unless libraries.include?('z')      # libgme + libarchive gzip
    libraries  << 'iconv'  unless libraries.include?('iconv')  # libarchive archive_string.c charset conversion
    libraries  << 'bz2'                                   # libarchive bzip2

    defines    << 'REWAMP_WITH_GME=1'
    defines    << 'VGM_YM2612_NUKED=1'    # select Nuked YM2612 core (default in Ym2612_Emu.h)
    search_paths << gme_root
    search_paths << "#{gme_root}/gme/ext"  # emu2413.h, panning.h
  ...
  if ENV['REWAMP_WITH_SID'] != '0'
    ...
    defines      << 'REWAMP_WITH_SID=1'
    defines      << 'HAVE_CXX23=1'
    search_paths += [
  ...
  if ENV['REWAMP_WITH_NSFPLAY'] != '0'
    defines      << 'REWAMP_WITH_NSFPLAY=1'
    defines      << 'REWAMP_NSF_OSCILLO_SIZE=4096'
    search_paths << '$(PODS_TARGET_SRCROOT)/../third_party'
    search_paths << '$(PODS_TARGET_SRCROOT)/../third_party/libnsfplay'
    ...
    prepare_parts << <<-'SH'
      ...
      # filter.cpp uses 'pi' which conflicts with deprecated extern double pi in the SDK.
      ...
      # Playlist sources (ppls.cpp references MAX which clashes with an SDK macro).
      ...
      for f in \
        "devices/Sound/legacy/emu2413.c" \
        "devices/Sound/legacy/emu2149.c" \
        "devices/Sound/legacy/emu2212.c"
      do
        name="$(echo "$f" | tr '/' '_' | sed 's/\\.c$//')"
        printf '#include "%s"\n#include "%s/%s"\n' "$RENAME_HDR" "$REL" "$f" > "$WRAPPERS_DIR/nsfplay_${name}.c.m"
      done
  ...
  if ENV['REWAMP_WITH_GBSPLAY'] != '0'
    defines      << 'REWAMP_WITH_GBSPLAY=1'
    search_paths << '$(PODS_TARGET_SRCROOT)/../third_party/libgbsplay'
    libraries    << 'z' unless libraries.include?('z')  # gzip GBS via zlib
  ...
  if ENV['REWAMP_WITH_HIGHLYEXP'] != '0'
    defines      << 'REWAMP_WITH_HIGHLYEXP=1'
    defines      << 'EMU_COMPILE'
    defines      << 'EMU_LITTLE_ENDIAN'
    defines      << 'HAVE_STDINT_H'
    search_paths << '$(PODS_TARGET_SRCROOT)/../third_party'
    libraries    << 'z' unless libraries.include?('z')  # libpsflib zlib
  ...
  # Requires prebuilt XCFramework:
  #   packages/rewamp_audio/scripts/build_vgmstream_ios.sh
  xcfw_path = File.join(__dir__, 'Libs/libvgmstream.xcframework')
  if ENV['REWAMP_WITH_VGMSTREAM'] != '0' && File.exist?(xcfw_path)
    vendored_frameworks << 'Libs/libvgmstream.xcframework'
    defines      << 'REWAMP_WITH_VGMSTREAM=1'
    search_paths << '$(PODS_TARGET_SRCROOT)/../third_party/vgmstream/src'
    search_paths << '$(PODS_TARGET_SRCROOT)/../third_party/vgmstream/ext_includes'

    # FFmpeg (ffmpeg-kit prebuilt frameworks) — same bypass as the macOS podspec:
    # libvgmstream.xcframework is built with VGM_USE_FFMPEG (build_vgmstream_ios.sh)
    # and leaves libav*/swr_ symbols unresolved; link + embed the dynamic
    # frameworks here to satisfy them. Built locally into ios/Libs/ffmpeg-kit
    # (gitignored). Absent → vgmstream still links, but FFmpeg-only codecs fail
    # to open at runtime. vendored_frameworks embeds + sets FRAMEWORK_SEARCH_PATHS
    # but does NOT add the link flag → we add -framework via extra_ldflags.
    ...
    search_paths << '$(PODS_TARGET_SRCROOT)/../src/vorbis_compat'
    search_paths << "#{vorbis_root}/ogg/include"
    search_paths << "#{vorbis_root}/vorbis/include"
    search_paths << "#{vorbis_root}/vorbis/lib"
  ...
  # Enabled by default; opt out with REWAMP_WITH_FURNACE=0. Mirrors the macOS
  # podspec + cmake rewamp_add_furnace(): compiles the exact engine set listed in
  ...
  # keep momo + HAVE_LOCALE (momo.c's SDL path is #ifdef ANDROID only). Compile as
  # plain C/C++ — NOT ObjC++: .mm pulls MacTypes.h whose legacy `pascal` macro
  ...
  if ENV['REWAMP_WITH_FURNACE'] != '0'
    ...
    search_paths << '$(PODS_TARGET_SRCROOT)/../third_party/furnace/extern/fmt/include'
    search_paths << '$(PODS_TARGET_SRCROOT)/../third_party/furnace/src/momo'
    search_paths << '$(PODS_TARGET_SRCROOT)/../third_party/furnace/extern/vgsound_emu-modified'
    search_paths << '$(PODS_TARGET_SRCROOT)/../third_party/furnace/extern/IconFontCppHeaders'
    search_paths << '$(PODS_TARGET_SRCROOT)/../third_party/furnace/extern/blip_buf'
    search_paths << '$(PODS_TARGET_SRCROOT)/../third_party/furnace/src/icon'
    search_paths << '$(PODS_TARGET_SRCROOT)/../third_party/furnace/src/modizer'   # FurnacePlayer.h for the plugin
    libraries    << 'c++' unless libraries.include?('c++')
    ...
    prepare_parts << <<-'SH'
      ...
      rm -rf "$WRAP"; mkdir -p "$WRAP"
      grep -oE '\$\{FURNACE_ROOT\}/[^"]+\.(c|cc|cpp)' "$SRCLIST" | sed 's#^\${FURNACE_ROOT}/##' | while read rel; do
  ...
  # Enabled by default; opt out with REWAMP_WITH_ZXTUNE=0. Mirrors the macOS podspec
  # and cmake rewamp_add_zxtune(): compiles the curated set from
  # third_party/libzxtune/zxtune_sources.cmake, each as its OWN translation unit.
  # Header-only boost 1.57 lives under the lib root. zxtune's target-private defines
  # (MODIZER — critical; boost flags; z80ex version macros) are emitted INTO each
  # wrapper so they don't leak pod-wide. boost 1.57 needs C++14 (auto_ptr etc.): the
  # app's ios/Podfile post_install forces -std=gnu++14 on Classes/zxtune_cores/*.cpp.
  # rewamp_zxtune_stubs.cpp satisfies force-loaded-but-unused symbols (boost::
  # filesystem, AYM dumpers); network_provider.cpp + zlib example.c are dropped from
  # the set (see zxtune_sources.cmake header).
  if ENV['REWAMP_WITH_ZXTUNE'] != '0'
    defines      << 'REWAMP_WITH_ZXTUNE=1'   # registry gate (rewamp_registry.c)
    search_paths << '$(PODS_TARGET_SRCROOT)/../third_party/libzxtune/include'
    search_paths << '$(PODS_TARGET_SRCROOT)/../third_party/libzxtune/src'
    search_paths << '$(PODS_TARGET_SRCROOT)/../third_party/libzxtune/3rdparty'
    search_paths << '$(PODS_TARGET_SRCROOT)/../third_party/libzxtune'
    search_paths << '$(PODS_TARGET_SRCROOT)/../third_party/libzxtune/3rdparty/z80ex/include'
    search_paths << '$(PODS_TARGET_SRCROOT)/../third_party/libzxtune/emscripten'
    search_paths << '$(PODS_TARGET_SRCROOT)/../third_party/libchpconv'  # chp2ym.h (.chp → YM3)
...
#define Z80EX_VERSION_STR "1.1.19pre1"'
  ...
  # --- UADE: Amiga custom-chip formats via 68k emulation (iOS = single-process) --
  # Enabled by default; opt out with REWAMP_WITH_UADE=0. Mirrors the macOS podspec + cmake rewamp_add_uade():
  # libzakalwe + bencode + libuade + the uadecore 68k emulator, each its OWN TU
  # (configure-duplicated names; cpuemu.c ×8 with PART_1..8). Built with
  # UADE_IN_PROCESS so uadecore runs in a pthread (NO fork — required on iOS); the
  # emulator TUs get uade_inprocess.h (exit()→longjmp shim). Feature macros are
  # emitted INTO each wrapper, not pod-wide. The UADE include dirs are scoped to
  # Classes/uade_cores/* via the app's ios/Podfile post_install (src/include/ holds
  # generic-named headers — memory.h/audio.h/… — that would shadow others pod-wide).
  if ENV['REWAMP_WITH_UADE'] != '0'
    defines    << 'REWAMP_WITH_UADE=1'   # registry + plugin gate (rewamp_registry.c)
    libraries  << 'c++' unless libraries.include?('c++')
...
#define _DARWIN_C_SOURCE 1'
  ...
  # Enabled by default; opt out with REWAMP_WITH_NEZ=0. Mirrors the macOS podspec +
  # cmake rewamp_add_nez(): one wrapper TU per curated source (device cores reuse
  # identical static fn names). Per-voice scope + notes + mute live in the vendored
  # cores (nezChan_output[] → m_voice_buff[] in format/audiosys.c, generic_mute_mask).
  if ENV['REWAMP_WITH_NEZ'] != '0'
    defines      << 'REWAMP_WITH_NEZ=1'   # registry gate (rewamp_registry.c)
    search_paths << '$(PODS_TARGET_SRCROOT)/../third_party/nez'
    search_paths << '$(PODS_TARGET_SRCROOT)/../third_party/nez/format'
    search_paths << '$(PODS_TARGET_SRCROOT)/../third_party/nez/device'
    search_paths << '$(PODS_TARGET_SRCROOT)/../third_party/nez/device/nes'
    search_paths << '$(PODS_TARGET_SRCROOT)/../third_party/nez/device/opl'
    search_paths << '$(PODS_TARGET_SRCROOT)/../third_party/nez/cpu'
    search_paths << '$(PODS_TARGET_SRCROOT)/../third_party/nez/cpu/wkmz80'
    search_paths << '$(PODS_TARGET_SRCROOT)/../third_party/nez/cpu/km6502'
  ...
  # Enabled by default; opt out with REWAMP_WITH_MAC=0. Mirrors the macOS podspec
  # + cmake rewamp_add_mac(): curated 26-file set from third_party/monkeyaudio/
  # mac_sources.cmake, one wrapper TU per file, MACLIB_COMPILE emitted into each
  # wrapper. MACLib include dirs are scoped per-file to Classes/mac_cores/* in
  # the app Podfiles' post_install (src/Shared has generic-named headers:
  # config.h, MD5.h, All.h — would shadow libarchive's config.h pod-wide).
  if ENV['REWAMP_WITH_MAC'] != '0'
    defines   << 'REWAMP_WITH_MAC=1'   # registry gate (rewamp_registry.c)
    libraries << 'c++' unless libraries.include?('c++')
  ...
  if ENV['REWAMP_WITH_V2M'] != '0'
    defines      << 'REWAMP_WITH_V2M=1'
    defines      << 'V2_RONAN=1'
    defines      << 'V2MPLAYER_SYNC_FUNCTIONS=1'
    search_paths << '$(PODS_TARGET_SRCROOT)/../third_party/v2redux/src'
  ...
  if ENV['REWAMP_WITH_SNDH'] != '0'
    defines      << 'REWAMP_WITH_SNDH=1'
    search_paths << '$(PODS_TARGET_SRCROOT)/../third_party/atariaudio'
  ...
  if ENV['REWAMP_WITH_LAZYUSF'] != '0'
    defines      << 'REWAMP_WITH_LAZYUSF=1'
    search_paths << '$(PODS_TARGET_SRCROOT)/../third_party/lazyusf'
    search_paths << '$(PODS_TARGET_SRCROOT)/../third_party'   # libpsflib/psflib.h
    libraries    << 'z' unless libraries.include?('z')
  ...
  if ENV['REWAMP_WITH_WONDERSWAN'] != '0'
    defines << 'REWAMP_WITH_WONDERSWAN=1'
  ...
  if ENV['REWAMP_WITH_HIGHLYQUIXOTIC'] != '0'
    defines      << 'REWAMP_WITH_HIGHLYQUIXOTIC=1'
    defines      << 'EMU_COMPILE'
    defines      << 'EMU_LITTLE_ENDIAN'
    defines      << 'HAVE_STDINT_H'
  ...
  # generic too (m68k.h/m68kcpu.h/arm.h etc) — scoped alongside the renames,
  # same per-file block.
  if ENV['REWAMP_WITH_HIGHLYTHEORITICAL'] != '0'
    defines      << 'REWAMP_WITH_HIGHLYTHEORITICAL=1'
    defines      << 'EMU_COMPILE'
    defines      << 'EMU_LITTLE_ENDIAN'
    defines      << 'HAVE_STDINT_H'
    defines      << 'USE_M68K'
  ...
  # same pod collide. No search path is added — every include here is either
  ...
  if ENV['REWAMP_WITH_TIATRACKER'] != '0'
    defines << 'REWAMP_WITH_TIATRACKER=1'
  ...
  if ENV['REWAMP_WITH_LIBPT3'] != '0'
    defines      << 'REWAMP_WITH_LIBPT3=1'
    search_paths << '$(PODS_TARGET_SRCROOT)/../third_party/libpt3'
  ...
  if ENV['REWAMP_WITH_PROWIZARD'] != '0'
    defines << 'REWAMP_WITH_PROWIZARD=1'   # gates rewamp_audio.c, pod-wide
  ...
  if ENV['REWAMP_WITH_SC68'] != '0'
    defines      << 'REWAMP_WITH_SC68=1'   # registry gate (rewamp_registry.c, pod-wide)
  ...
  # iOS twin of the macOS block. NOTE: iOS podspec uses `defines`/`search_paths`,
  # NOT macOS's `preprocessor`/`header_dirs` (mixing them up crashes pod
  # install). Headless defines live inside each
  # generated wrapper; include dirs scoped per-file to Classes/sunvox_cores/* in
  # the app ios/Podfile. COLOR16BITS (not 32) on iOS, per the SDK Makefile. iOS
  # also compiles sound_ios.mm (the AudioToolbox/AVFoundation backend).
  if ENV['REWAMP_WITH_SUNVOX'] != '0'
    defines << 'REWAMP_WITH_SUNVOX=1'   # registry gate (rewamp_registry.c, pod-wide)
    prepare_parts << <<-'SH'
      CLASSES="$(pwd)/Classes"
      ...
      # Apple file backend + iOS audio backend (ObjC++).
      { printf "$DEFS"; printf '#include "%s/lib_sundog/file/file_apple.mm"\n' "$REL"; } > "$WRAP/lib_sundog_file_file_apple__mm.mm"
      { printf "$DEFS"; printf '#include "%s/lib_sundog/sound/sound_ios.mm"\n' "$REL"; } > "$WRAP/lib_sundog_sound_sound_ios__mm.mm"
  ...
  if ENV['REWAMP_WITH_PXTONE'] != '0'
    defines      << 'REWAMP_WITH_PXTONE=1'
    search_paths << '$(PODS_TARGET_SRCROOT)/../third_party/libpixel/pxtone'
  ...
  if ENV['REWAMP_WITH_PMD'] != '0'
    defines      << 'REWAMP_WITH_PMD=1'
    search_paths << '$(PODS_TARGET_SRCROOT)/../third_party/pmdmini/src'
  ...
  if ENV['REWAMP_WITH_GSF'] != '0'
    defines      << 'REWAMP_WITH_GSF=1'
    defines      << 'LINUX=1'                # VBA tree's non-Windows path
    search_paths << '$(PODS_TARGET_SRCROOT)/../third_party/gsf'
    search_paths << '$(PODS_TARGET_SRCROOT)/../third_party/gsf/VBA'
    search_paths << '$(PODS_TARGET_SRCROOT)/../third_party/gsf/libresample/include'
    search_paths << '$(PODS_TARGET_SRCROOT)/../third_party/gsf/libresample/src'
  ...
  if ENV['REWAMP_WITH_VIO2SF'] != '0'
    ...
    search_paths << '$(PODS_TARGET_SRCROOT)/../third_party/vio2sf/include'
  ...
  if ENV['REWAMP_WITH_NCSF'] != '0'
    defines      << 'REWAMP_WITH_NCSF=1'
    search_paths << '$(PODS_TARGET_SRCROOT)/../third_party/sseqplayer/include'
  ...
  if ENV['REWAMP_WITH_ADPLUG'] != '0'
    defines      << 'REWAMP_WITH_ADPLUG=1'   # registry gate (rewamp_registry.c, pod-wide)
  ...
  if ENV['REWAMP_WITH_MIDI'] != '0'
    defines      << 'REWAMP_WITH_MIDI=1'
    defines      << 'FLUIDLITE_STATIC=1'
    defines      << 'REWAMP_VOICE_CAPTURE=1'   # per-channel scope hooks in fluid_synth.c
    search_paths << '$(PODS_TARGET_SRCROOT)/../third_party/fluidlite/include'
    search_paths << '$(PODS_TARGET_SRCROOT)/../third_party/fluidlite/src'
  ...
  if ENV['REWAMP_WITH_HVL'] != '0'
    defines     << 'REWAMP_WITH_HVL=1'
    search_paths << '$(PODS_TARGET_SRCROOT)/../third_party/hivelytracker'
  ...
  if ENV['REWAMP_WITH_ASAP'] != '0'
    defines   << 'REWAMP_WITH_ASAP=1'
    search_paths << '$(PODS_TARGET_SRCROOT)/../third_party/asap'
  ...
  # Enabled by default; opt out with REWAMP_WITH_PROJECTM=0. Mirrors the macOS
  # podspec: the curated projectm_sources.txt set, one TU per file (cores reuse
  # static fn names), projectM's defines baked into each generated wrapper (keeps
  # <filesystem> out of the build system's escaping), renderer as its own TU with
  # the GLES header first. The shape fast path (framebuffer fetch) is picked at
  # RUNTIME from the GL extensions actually present — our ANGLE build exposes
  # GL_EXT_shader_framebuffer_fetch on every Apple GPU (family 1+) via
  # patches/angle/0001-metal-EXT_shader_framebuffer_fetch.patch. MetalANGLE,
  # which this replaced, never advertised it, so iOS always took the slow path.
  # NOTE: projectM's include dirs are scoped PER-FILE to Classes/projectm_cores/*
  # in the app Podfile's post_install (NOT pod-wide) — its src/ and vendor/ carry
  # generic header names (Shader.hpp, Factory.cpp, a compiler/filesystem layer)
  # that would shadow other plugins' headers pod-wide.
  if ENV['REWAMP_WITH_PROJECTM'] != '0'
    ...
    defines << 'REWAMP_PM_PROFILE=1' if ENV['REWAMP_PM_PROFILE'] == '1'
    defines << 'REWAMP_WITH_PROJECTM=1'
  ...
  s.libraries           = libraries          unless libraries.empty?

  unless defines.empty?
    xcconfig['GCC_PREPROCESSOR_DEFINITIONS'] = '$(inherited) ' + defines.join(' ')
  end
  unless search_paths.empty?
    xcconfig['HEADER_SEARCH_PATHS'] += ' ' + search_paths.join(' ')
  end
  unless extra_ldflags.empty?
    xcconfig['OTHER_LDFLAGS'] = '$(inherited) ' + extra_ldflags.join(' ')
  end
