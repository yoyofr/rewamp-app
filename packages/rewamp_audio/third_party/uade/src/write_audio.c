					       ...
					       const int fd)
{
	...
	if (fname == NULL) {
		w->f = fdopen(fd, "wb");
		if (w->f == NULL) {
			fprintf(stderr, "error: Can not open fd %d to write "
				"audio: %d (%s)\n", fd, errno,
				strerror(errno));
			goto out;
		}
		...
		w->f = fopen(fname, "wb");
		if (w->f == NULL) {
			fprintf(stderr, "error: Can not open %s to write "
				"audio\n", fname);
			goto out;
		}
	}
			    ...
			    const unsigned long tdelta)
{
		...
		for (et = 1 ; et < PET_MAX_ENUM; et++) {
			if (ce->active_events[et]) {
				...
				if (uade_atomic_fwrite(&paula_frame,
						       sizeof(paula_frame),
						       1, w->f) != 1) {
                                    if (ferror(w->f) && errno != ECONNRESET) {
					    fprintf(stderr,
						    "error: Unable to write paula event frame: %s\n", strerror(errno));
				    }
				}
			}
		}
	...
	if (w->started) {
		if (uade_atomic_fwrite(&frame, sizeof(frame), 1, w->f) != 1)
			if (ferror(w->f) && errno != ECONNRESET) {
				fprintf(stderr, "uade: Unable to write paula frame: %s\n", strerror(errno));
			}
	}
}
	...
	struct uade_write_audio *w, const int left, const int right)
{
	...
	if (uade_atomic_fwrite(&frame, sizeof(frame), 1, w->f) != 1) {
                if (ferror(w->f) && errno != ECONNRESET) {
			fprintf(stderr, "error: Unable to write paula event frame: %s\n", strerror(errno));
                }
	}
	pef->channel = 1;
