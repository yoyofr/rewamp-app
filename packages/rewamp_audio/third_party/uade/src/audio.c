...
/* Per-voice oscilloscope + note capture, wired to rewamp's generic voice
 * framework. Relative path resolves the same whether compiled directly (cmake)
 * or via the Apple pod wrapper (the wrapper #includes this real file). */
...
static void check_sound_buffers (void)
{
	...
	if (bytes == uadecore_read_size) {
	    ...
	    sndbufpt = sndbuffer;
	    //YOYOFR: decay note/vol each output buffer so the overlays release
...
//TODO: MODIZER changes start / YOYOFR
/* Push one oscilloscope sample per Paula voice (0..3) into rewamp's per-voice
 * ring buffers, and mirror the UI mute mask into UADE's own audio_mutemask so
 * the existing ismuted() path mutes the mix. Called once per output sample by
 * each sample handler with the four pre-mix voice outputs. A muted voice is
 * captured as silence so the scope shows it flat (matches Modizer). */
static inline void uade_capture_voices(int o0, int o1, int o2, int o3)
{
    audio_mutemask = (int)generic_mute_mask;   /* UI mute → UADE mix */

    const int outv[4] = { o0, o1, o2, o3 };
    const int64_t step = (int64_t)1 << MODIZER_OSCILLO_OFFSET_FIXEDPOINT;
    const int mask = SOUND_BUFFER_SIZE_SAMPLE * 4 * 4 - 1;

    for (int v = 0; v < 4; v++) {
        if (!m_voice_buff[v]) continue;
        int s = ismuted(v) ? 0 : outv[v];
        int idx = (int)((m_voice_current_ptr[v] >> MODIZER_OSCILLO_OFFSET_FIXEDPOINT) & mask);
        m_voice_buff[v][idx] = LIMIT8((s >> 6) * 0.7);
        m_voice_current_ptr[v] += step;
        if ((m_voice_current_ptr[v] >> MODIZER_OSCILLO_OFFSET_FIXEDPOINT) >= SOUND_BUFFER_SIZE_SAMPLE * 4 * 4)
            m_voice_current_ptr[v] -= (int64_t)(SOUND_BUFFER_SIZE_SAMPLE * 4 * 4) << MODIZER_OSCILLO_OFFSET_FIXEDPOINT;
    }
}
//TODO: MODIZER changes end / YOYOFR
...
static void sample16s_handler (void)
{
    ...
    uade_capture_voices(output[0], output[1], output[2], output[3]); //YOYOFR
...
void update_audio (void)
{
     ...
     * constant d'un coup de batterie au suivant. */
    {
        ...
        for (int i = 0; i < 4; i++) {
            int output = (int)((audio_channel[i].current_sample * audio_channel[i].vol) & audio_channel[i].adk_mask);
            if (audio_channel[i].per && audio_channel[i].per < 65535 &&
                audio_channel[i].vol && audio_channel[i].state &&
