...
/* See uade_inprocess.h: exit() in any uadecore TU longjmp()s here so the pthread
   running uadecore_main() unwinds and returns instead of killing the process. */
jmp_buf uadecore_exit_jmp;
void uadecore_exit(int code)
{
    longjmp(uadecore_exit_jmp, code ? code : 256);
}
#endif
...
{
#ifdef UADE_IN_PROCESS
    if (setjmp(uadecore_exit_jmp))
        return 0; /* an exit() inside uadecore unwound here — end the thread */
#endif
