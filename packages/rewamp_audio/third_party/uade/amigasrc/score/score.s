...
* rewamp: renumbered 18/19 -> 24/25. The modern upstream protocol assigns
* 18=RESERVED_0 and 19-23 to STATE_DETECTION/TEST_LOGGING/DEBUG messages,
* and our C side (src/uade.c) speaks that protocol - keeping Wothke's
* original numbers would have made icon.library calls arrive as bogus
* state-detection messages. Kept in sync with src/include/amigamsg.h.
AMIGAMSG_ICON_LOAD	equ	24
AMIGAMSG_ICON_TOOLTYPE	equ	25
...
start	* set super stack and user stack
	...
	move.l	$110,a0
	move.l	#'WALL',(a0)	/* user stack guard (rewamp: upstream C checks WALL on both) */
	move.l	a0,usp
