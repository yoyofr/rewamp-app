...
#ifdef REWAMP_VOICE_CAPTURE
/* rewamp per-MIDI-channel capture hooks (implemented in rewamp_plugin_midi.c):
 * - _muted(ch):   1 = remove this channel's voices from the audible mix
 *                 (their envelopes keep running for instant unmute)
 * - _voice(...):  one voice's contribution to the current 64-frame block
 *                 (dl/dr may be NULL for a muted voice = capture silence)
 * - _block_done:  end of the block; flush accumulators into the voice rings */
extern int  rewamp_fluid_channel_muted(int midi_chan);
extern void rewamp_fluid_capture_voice(int midi_chan, const float* dl,
                                       const float* dr, int len);
extern void rewamp_fluid_block_done(int len);
#endif
...
#ifdef REWAMP_VOICE_CAPTURE
      {
        int mchan = fluid_channel_get_num(fluid_voice_get_channel(voice));
        fluid_real_t preL[FLUID_BUFSIZE], preR[FLUID_BUFSIZE];
        fluid_real_t preRev[FLUID_BUFSIZE], preCho[FLUID_BUFSIZE];
        FLUID_MEMCPY(preL, left_buf,  sizeof(preL));
        FLUID_MEMCPY(preR, right_buf, sizeof(preR));
        if (reverb_buf) FLUID_MEMCPY(preRev, reverb_buf, sizeof(preRev));
        if (chorus_buf) FLUID_MEMCPY(preCho, chorus_buf, sizeof(preCho));

        fluid_voice_write(voice, left_buf, right_buf, reverb_buf, chorus_buf);

        if (rewamp_fluid_channel_muted(mchan)) {
          /* Silence this voice everywhere (dry + fx sends) but keep its
           * envelope state advancing for a click-free instant unmute. */
          FLUID_MEMCPY(left_buf,  preL, sizeof(preL));
          FLUID_MEMCPY(right_buf, preR, sizeof(preR));
          if (reverb_buf) FLUID_MEMCPY(reverb_buf, preRev, sizeof(preRev));
          if (chorus_buf) FLUID_MEMCPY(chorus_buf, preCho, sizeof(preCho));
          rewamp_fluid_capture_voice(mchan, NULL, NULL, FLUID_BUFSIZE);
        } else {
          float dl[FLUID_BUFSIZE], dr[FLUID_BUFSIZE];
          int k;
          for (k = 0; k < FLUID_BUFSIZE; k++) {
            dl[k] = (float)(left_buf[k]  - preL[k]);
            dr[k] = (float)(right_buf[k] - preR[k]);
          }
          rewamp_fluid_capture_voice(mchan, dl, dr, FLUID_BUFSIZE);
        }
      }
#else
      fluid_voice_write(voice, left_buf, right_buf, reverb_buf, chorus_buf);
...
//TODO: MODIZER changes start / YOYOFR
#ifdef REWAMP_VOICE_CAPTURE
  rewamp_fluid_block_done(FLUID_BUFSIZE);
#endif
//TODO: MODIZER changes end / YOYOFR
