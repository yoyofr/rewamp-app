	...
	while (buflen--)
	{
		...
		if (bufp)
		{
            ...
            for (int c=0;c<4+11;c++) {
                ...
                // rewamp: this loop always iterates 15 (4+11) channels, but the
                // plugin only allocates m_voice_buff[] for the real voice count of
                // the current format (HES=6, SGC=4/15). Guard against writing into
                // an unallocated (NULL) voice buffer → otherwise crash on load.
                if (m_voice_buff[c]) {
