...
#include <cstring>
#include "nsfplay.h"
#ifndef REWAMP_NSF_OSCILLO_SIZE
#define REWAMP_NSF_OSCILLO_SIZE SOUND_BUFFER_SIZE_SAMPLE
#endif
      ...
      UpdateInfo();
        ...
        lpf_voices[2].FastRender(buf_voices);
            ...
            for (int i=0;i<m_voice_currentChannel;i++) {
                ...
                if ((m_voice_current_ptr[i]>>MODIZER_OSCILLO_OFFSET_FIXEDPOINT)>=REWAMP_NSF_OSCILLO_SIZE) m_voice_current_ptr[i]-=(REWAMP_NSF_OSCILLO_SIZE)<<MODIZER_OSCILLO_OFFSET_FIXEDPOINT;
            }
        //TODO:  MODIZER changes end / YOYOFR
    }
