 ...
 */
  ...
  int NSF::GetPlayTime ()
  {
    ...
    // YOYOFR: honor a .m3u / playlist time_in_ms (set by PLSITEM/LoadFile)
    // as a fallback before the generic 5-min default. Original patch dropped
    // this so plain NSFs used default_playtime; restored so sibling-.m3u loop
    // metadata (filename::NSF,song,title,time,loop,fade,loopcount) takes effect.
    else if (time_in_ms >= 0) return time_in_ms;
