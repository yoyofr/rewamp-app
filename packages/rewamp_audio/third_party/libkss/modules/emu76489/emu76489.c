...
//  sng->out = sng->ch_out[0] + sng->ch_out[1] + sng->ch_out[2] + sng->ch_out[3];
    ...
    if (m_voicesForceOfs>=0) {
        int val=0;
        for (int i = 0; i < 4; i++) {
            if (!(generic_mute_mask&((int64_t)1<<(m_voicesForceOfs+i)))) val=sng->ch_out[i];
            else val=0;
...
mix_output_stereo (SNG * sng, int32_t out[2])
{
  ...
  if(sng->stereo & 0x08) {
      if (!(generic_mute_mask&((int64_t)1<<(m_voicesForceOfs+3)))) out[1] += sng->ch_out[3];
  }
        
  for (i = 0; i < 3; i++)
  {
    if ((sng->stereo >> (i+4)) & 0x01) {
        if (!(generic_mute_mask&((int64_t)1<<(m_voicesForceOfs+i)))) out[0] += sng->ch_out[i];
    }
    if ((sng->stereo >> i) & 0x01) {
        if (!(generic_mute_mask&((int64_t)1<<(m_voicesForceOfs+i)))) out[1] += sng->ch_out[i];
    }
  }
