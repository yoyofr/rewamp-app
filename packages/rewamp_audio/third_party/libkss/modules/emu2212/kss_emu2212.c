...
mix_output(SCCKSS * scc) {
    ...
    //TODO:  MODIZER changes start / YOYOFR
    for (int i=0;i<5;i++)
        if (!(generic_mute_mask&((int64_t)1<<(m_voicesForceOfs+i)))) scc->out += scc->ch_out[i];
  //scc->out = scc->ch_out[0] + scc->ch_out[1] + scc->ch_out[2] + scc->ch_out[3] + scc->ch_out[4];
    ...
    if (m_voicesForceOfs>=0) {
        int val=0;
        for (int i = 0; i < 5; i++) {
            if (!(generic_mute_mask&((int64_t)1<<(m_voicesForceOfs+i)))) val=scc->ch_out[i]+m_voice_buff_adjustement;
            else val=0;
