...
// Per-voice scope gain: the emu cores write ~1/4-scale int8 (a full-volume PSG
// tone peaks ~32/127), so the oscilloscope traces read very weak. Boost before
// the final LIMIT8 clamp. Tunable — raise if still faint, lower if loud chips
// (SCC/OPLL) flat-top on the scope.
#define KSS_SCOPE_GAIN 3
...
#define neg(x) ((x < 0) ? x : 0)
  ...
  for (i = 0; i < KSS_DEVICE_MAX; i++) {
    ...
    //TODO:  MODIZER changes start / YOYOFR
    // rewamp feeds a CONTINUOUS per-voice ring (Modizer instead reset each voice
    // pointer to 0 and memset the whole voice buffer every block). Two things must
    // happen per calc for a clean scope:
    //   1. snapshot each voice's write pointer into m_voice_prev_current_ptr so the
    //      post-loop below filters/overwrites exactly the span written this call
    //      (it starts from prev — never set elsewhere in rewamp);
    //   2. pre-clear that upcoming span so the emu cores' peak-hold write
    //      (abs(old)<abs(new)) cannot keep a louder STALE sample from a previous
    //      ring wrap — that ghost is what flickers as overlapping traces.
    for (int _v = 0; _v < m_genNumVoicesChannels; _v++) {
      m_voice_prev_current_ptr[_v] = m_voice_current_ptr[_v];
      int64_t _p = m_voice_current_ptr[_v] >> MODIZER_OSCILLO_OFFSET_FIXEDPOINT;
      for (uint32_t _k = 0; _k < length; _k++)
        m_voice_buff[_v][(_p + _k) & (SOUND_BUFFER_SIZE_SAMPLE * 4 * 2 - 1)] = 0;
    }
    //TODO:  MODIZER changes end / YOYOFR
...
//      }
    ...
    for (int jj=0;jj<m_genNumVoicesChannels;jj++) {
        ...
        for (int i=0;i<length;i++) {
            ...
            val*=KSS_SCOPE_GAIN;   //YOYOFR: lift the weak per-voice scope amplitude
