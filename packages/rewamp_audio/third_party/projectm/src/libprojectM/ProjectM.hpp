...
public:
    ...
    void PreLoadPresetFile(const std::string& presetFilename,uint32_t *warpP,uint32_t *compP,uint64_t *randSeed=nullptr);
    void LoadPreLoadPresetFile(const std::string& presetFilename,uint32_t warpP,uint32_t compP, bool smoothTransition,uint64_t randSeed=0);

    /**
     * @brief How many randNN textures the preload behind @p randSeed resolved.
     * Observability for the offline probe; the apply path consumes the entry.
     */
    int PreloadedRandomTextureCount(uint64_t randSeed) const;

    /**
     * @brief GL program ids of the built-in transition shaders (rewamp).
     * See TransitionShaderManager::ShaderPrograms.
     */
    std::vector<uint32_t> GetTransitionShaderPrograms() const;

    /**
     * @brief Pins the transition pattern to one index, -1 for random (rewamp).
     */
    /**
     * @brief Loads a texture into the manager's cache ahead of time (rewamp).
     *
     * Meant for the preload worker: textures ARE share-group objects, so one
     * warmed on the second context is the very object the render thread will
     * find cached. @p colorKey < 0 loads the plain texture.
     */
    void PreloadTexture(const std::string& name, int colorKey);
...
private:
    ...
    //! randNN textures a preload resolved, keyed by the seed handed back to the
    //! caller. The seed is the only thing that crosses the C API; the names stay
    //! in-process, because the render thread cannot re-draw them reliably (its
    //! scanned file list is not the worker's - see PresetState::randomTextureFiles).
    //! Bounded: a preload whose preset never gets applied would leak otherwise.
    std::map<uint64_t, std::map<int, std::string>> m_preloadRandomTextures;
    std::vector<uint64_t> m_preloadRandomTextureOrder; //!< Insertion order of the above, oldest first.
