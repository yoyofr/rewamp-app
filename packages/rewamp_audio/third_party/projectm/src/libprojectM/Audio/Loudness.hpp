...
public:
     ...
     */
    /**
     * @param hasReference Whether a long-term reference exists. Decided ONCE for
     *        all three bands by the caller, never per band - see PCM::Add.
     */
    void Update(const std::array<float, SpectrumSamples>& spectrumSamples, double secondsSinceLastFrame, uint32_t frame, bool hasReference);

    /**
     * @brief The long-term average, so the caller can decide whether a reference
     * exists across all bands at once.
     */
    auto LongAverage() const -> float { return m_longAverage; }
...
private:
     ...
     */
    void UpdateBandAverage(double secondsSinceLastFrame, uint32_t frame, bool hasReference);
