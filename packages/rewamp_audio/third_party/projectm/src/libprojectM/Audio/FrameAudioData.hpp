...
public:
    ...
    //! Un-equalized spectrum, used by the get_fft() family of shader functions.
    //! The equalized spectrum above zeroes the lowest bin and tilts the low end, which is
    //! fine for beat detection but useless as a full-range spectrum display.
    std::array<float, SpectrumSamples> shaderSpectrumLeft;
    std::array<float, SpectrumSamples> shaderSpectrumRight;
};
