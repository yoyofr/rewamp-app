...
private:
     ...
     * Updates FFT data
     */
    void UpdateSpectrum(MilkdropFFT& fft, const WaveformBuffer& waveformData, SpectrumBuffer& spectrumData);
    ...
    // Un-equalized spectrum data, used by the get_fft() shader functions.
    SpectrumBuffer m_shaderSpectrumL{0.f}; //!< Left-channel un-equalized spectrum data.
    SpectrumBuffer m_shaderSpectrumR{0.f}; //!< Right-channel un-equalized spectrum data.

    MilkdropFFT m_fft{WaveformSamples, SpectrumSamples, true};              //!< Spectrum analyzer instance.
    MilkdropFFT m_shaderFft{WaveformSamples, SpectrumSamples, false};       //!< Un-equalized spectrum analyzer for shader use.
