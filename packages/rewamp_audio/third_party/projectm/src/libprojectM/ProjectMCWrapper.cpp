...
void projectm_preload_preset_file(projectm_handle instance, const char* filename,uint32_t *warpP,uint32_t *compP,uint64_t *randSeed)
{
    auto projectMInstance = handle_to_instance(instance);
    projectMInstance->PreLoadPresetFile(filename,warpP,compP,randSeed);
}

void projectm_loadpreload_preset_file(projectm_handle instance, const char* filename,uint32_t warpP,uint32_t compP, bool smoothTransition,uint64_t randSeed)
{
    auto projectMInstance = handle_to_instance(instance);
    projectMInstance->LoadPreLoadPresetFile(filename,warpP,compP,smoothTransition,randSeed);
}

int projectm_preloaded_random_texture_count(projectm_handle instance, uint64_t randSeed)
{
    auto projectMInstance = handle_to_instance(instance);
    return projectMInstance->PreloadedRandomTextureCount(randSeed);
}

void projectm_preload_texture(projectm_handle instance, const char* name, int color_key)
{
    if (name == nullptr) return;
    auto projectMInstance = handle_to_instance(instance);
    projectMInstance->PreloadTexture(name, color_key);
}

void projectm_set_transition_index(projectm_handle instance, int index)
{
    auto projectMInstance = handle_to_instance(instance);
    projectMInstance->SetTransitionIndex(index);
}

int projectm_get_transition_count(projectm_handle instance)
{
    auto projectMInstance = handle_to_instance(instance);
    return projectMInstance->TransitionCount();
}

int projectm_get_transition_shader_programs(projectm_handle instance, uint32_t* out, int max)
{
    auto projectMInstance = handle_to_instance(instance);
    const auto programs = projectMInstance->GetTransitionShaderPrograms();
    const int count = static_cast<int>(programs.size());
    if (out != nullptr)
    {
        for (int i = 0; i < count && i < max; ++i)
        {
            out[i] = programs[static_cast<size_t>(i)];
        }
    }
    return count;
}
