...
void CustomWaveform::Draw(const PerFrameContext& presetPerFrameContext)
{
    ...
    // Compté APRÈS le code par frame — qui peut assigner `samples`; MilkDrop le
    // lit avant, nous après, et c'est la seule divergence qu'on garde ici.
    //
    // `- m_sep` était calculé AVANT ce code puis ÉCRASÉ par la relecture, donc
    // jamais appliqué. Pour une vague spectre c'est sans effet visible (`t`
    // compense: même dernier index, plus de points), mais pour une vague à DEUX
    // canaux les offsets valent `(max - n)/2 ∓ sep/2`, et un `n` non réduit les
    // décale vers le bas ET rallonge la lecture: à samples=480 et sep=30 le
    // dernier index atteint 494 dans un tableau de 480. Lecture hors bornes.
    int sampleCount = std::min(maxSampleCount,
                               static_cast<int>(*m_perFrameContext.samples)) - m_sep;
