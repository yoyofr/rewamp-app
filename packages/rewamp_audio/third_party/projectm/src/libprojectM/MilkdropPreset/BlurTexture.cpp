                                          ...
                                          Values& blurMax)
{
    ...
    // also, if min-max are close to each other, PUSH THEM APART - which is the
    // whole point of the clause, and what the max lines below do with a PLUS.
    // All three carried a minus, so min and max came out EQUAL and the caller's
    // scale = 1/(max-min) divided by zero. MilkDrop's own code (milkdropfs.cpp,
    // GetSafeBlurMinMax) uses avg +/- fMinDist*0.5f. No preset of the five packs
    // reaches it - the defaults are blurN_min=0/blurN_max=1, a gap of 1 against
    // a 0.1 threshold - so this never fired in practice; it would have, the day
    // a preset set its blur range narrow.
