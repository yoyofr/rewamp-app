...
#include "FramebufferFetch.hpp"   // rewamp: runtime framebuffer-fetch gate
...
void PresetState::LoadShaders()
{
    ...
    // The texturedBordered shader blends inside the shader by reading the
    // framebuffer, which needs GL_EXT_ or GL_ARM_shader_framebuffer_fetch (see
    // CustomShape's fast path). Availability is a per-GPU/driver fact, so this is
    // a RUNTIME decision (rewamp — FramebufferFetch.hpp): compiling a shader whose
    // "#extension … : require" the driver doesn't have would fail outright.
    // Neither extension → the program is never built and CustomShape falls back to
    // the standard multi-draw blend path.
