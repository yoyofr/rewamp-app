...
MilkdropPreset::MilkdropPreset(const std::string& absoluteFilePath)
    ...
    , m_border(m_state)
{
    // Only used to label shader-compile errors - see PresetState::presetName.
    m_state.presetName = ParseFilename(absoluteFilePath);
...
void MilkdropPreset::SetRandomTextures(uint64_t seed, const std::map<int, std::string>& resolvedFiles) {
    m_state.randomSeed = seed;
    m_state.randomTextureFiles = resolvedFiles;
}
...
void MilkdropPreset::RenderFrame(const libprojectM::Audio::FrameAudioData& audioData, const Renderer::RenderContext& renderContext)
{
    ...
    // Feed the spectrum/waveform textures read by get_fft()/get_wave(). No-op unless a
    // shader of this preset asked for them.
    m_state.audioTexture.Update(audioData,
                                renderContext.fps > 0.0f ? 1.0 / renderContext.fps : 1.0 / 60.0,
                                m_state.fftAttack, m_state.fftDecay);
    ...
    // Update blur textures from the PREVIOUS buffer, and do not "fix" this to
    // the current one: the position of this call in the frame is NOT what
    // decides the source.
    //
    // MilkDrop warps VS[0] into VS[1], and its BlurPasses() then blurs VS[0] -
    // the warp's SOURCE, i.e. the previous frame's finished image, custom waves
    // and shapes included. Its own comment ("it blurs the current frame after
    // warp") describes the call's position, not its input, and reading it as
    // the input is the trap. That is exactly what happened here: this line was
    // changed to m_currentFrameBuffer, which blurs the warp's OUTPUT instead.
    //
    // The preset that shows it is "martin - space debris": its spectrum is a
    // column of quads drawn at x >= 0.97 AFTER this point, its warp then wipes
    // that strip (ret *= uv.x<0.97), and its composite reads the column back
    // through GetBlur1/2/3. Blur the warp output and the strip is always the
    // wiped one, so the spectrum is lit but frozen - measured offline, the
    // column's frame-to-frame deviation collapses to ~0.1 on a mean of 21.
    //
    // Upstream fix (projectM 4.1.6), contributed after exactly this
    // investigation in Modizer; keep in sync with it.
    const auto sourceImage = m_framebuffer.GetColorAttachmentTexture(m_previousFrameBuffer, 0);
    assert(sourceImage.get());
    m_state.blurTexture.Update(*sourceImage, m_perFrameContext);

    // The sprite seam. MilkDrop stamps user sprites into the frame HERE: after the warp
    // mesh, before the shapes, the border and the composite. Three observations taken
    // against MilkDrop 3 all say the same thing and only fit this point: the sprite never
    // warps (it is stamped after the warp of its own frame), the border and the preset's
    // shapes draw OVER it, and it still goes through the composite - which is what turns
    // "the glass bead game 001" into its balloons.
    //
    // Drawn after the composite instead (where the sprite manager used to run, at the end
    // of ProjectM::RenderFrame), the stamp only reached the preset image in time for the
    // NEXT frame, so it was warped once before ever being seen: bead 001 came out as an X
    // where MilkDrop draws straight bars.
    if (renderContext.spriteManager != nullptr)
    {
        renderContext.spriteManager->DrawEmbedded();
    }
