...
#include "FramebufferFetch.hpp"   // rewamp: EXT vs ARM framebuffer fetch
...
uniform float4   _c14;          // pointer: .xy = position, .z = held, .w = clicked
                                // (all -1: the visualizer has no pointer)
...
// MilkDrop3 / BeatDrop / Milkwave additions. The pointer reads as "outside the
// window" (-1) because the visualizer has none; presets that use it degrade to
// their idle behaviour instead of failing to compile.
#define mouse _c14
...
// Same shader for GL_ARM_shader_framebuffer_fetch (rewamp addition). The two
// extensions are NOT source-compatible: EXT re-reads the `inout` output, while
// ARM gives a plain `out` plus the read-only built-in gl_LastFragColorARM for
// the destination colour. Everything else is identical.
static std::string kTexturedBorderedDrawFragmentShaderArmGlsl330 = R"(
...
void main(){
    ...
    // additive mode or not ?
    if (fragment_renderFlag.x==1.0) color=newColor*newColor.a + dst; //SRC_ALPHA / ONE
    else color=newColor*newColor.a + dst*(1.0 - newColor.a); //SRC_ALPHA / ONE_MINUS_SRC_ALPHA
}
)";
...
// Bordered shader: pick the source that matches the extension actually present
// (rewamp — see FramebufferFetch.hpp). Never called when neither is available:
// PresetState skips compiling this program entirely in that case.
std::string MilkdropStaticShaders::GetTexturedBorderedDrawFragmentShader()
{
    return AddVersionHeader(FramebufferFetch() == FramebufferFetchMode::Arm
                                ? kTexturedBorderedDrawFragmentShaderArmGlsl330
                                : kTexturedBorderedDrawFragmentShaderGlsl330);
}
