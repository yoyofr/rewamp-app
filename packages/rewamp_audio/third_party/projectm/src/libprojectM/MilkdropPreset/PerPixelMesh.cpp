        ...
        for (int x = 0; x <= m_gridSizeX; x++)
        {
            ...
            if (perPixelContext.perPixelCodeHandle)
            {
                *perPixelContext.x = static_cast<double>(curVertex.X() * 0.5f * presetState.renderContext.aspectX + 0.5f);
                // MilkDrop writes -0.5f here (milkdropfs.cpp:1757, var_pv_y =
                // vert.y * -0.5f * aspectY + 0.5f). PM_PPY=1 restores that sign
                // as an A/B in one binary: it flips the `y` every per-pixel
                // equation reads, so it changes the warp of EVERY preset whose
                // per-pixel code is not symmetric in y.
                //
                // TESTED AND RULED OUT as the cause of "the glass bead game
                // 001" rendering a pinched X where MilkDrop renders rounded
                // lens shapes: flipping it mirrors the X, it does not become
                // the other shape. Whether the sign should be flipped anyway,
                // for fidelity, is a separate question nobody has measured -
                // it would need a preset whose per-pixel code is asymmetric in
                // y and a MilkDrop capture to compare against.
                static const bool ppyFlip = (getenv("PM_PPY") != nullptr);
                *perPixelContext.y = static_cast<double>(
                    (ppyFlip ? -0.5f : 0.5f) * curVertex.Y() * presetState.renderContext.aspectY + 0.5f);
