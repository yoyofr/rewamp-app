...
AltMilkdropPreset::AltMilkdropPreset(const std::string& absoluteFilePath)
    ...
    //, m_border(m_state)
{
    // Only used to label shader-compile errors - see AltPresetState::presetName.
    m_state.presetName = ParseFilename(absoluteFilePath) + " (precomp)";
...
void AltMilkdropPreset::SetRandomTextures(uint64_t seed, const std::map<int, std::string>& resolvedFiles) {
    // The preload DRAWS, it is not told. Only the seed is used here; the files
    // are the output of this pass, read back by GetRandomTextures.
    m_state.randomSeed = seed;
}

void AltMilkdropPreset::GetRandomTextures(uint64_t& seed, std::map<int, std::string>& resolvedFiles) const {
    seed = m_state.randomSeed;
    resolvedFiles = m_state.randomTextureFiles;
}
