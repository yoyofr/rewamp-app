...
public:
    ...
    //! See PresetState: zero means "the preload produced nothing", and that is
    //! exactly what the caller tests. Leaving these uninitialised handed stack
    //! garbage to the fast path.
...
//    BlurTexture blurTexture;                      //!< The blur textures used in this preset. Contents depend on the shader code using GetBlurX().
    ...
    //! Seed for the randNN draws, drawn once per preload job.
    uint64_t randomSeed{0};
    //! randNN slot -> the texture file this preload resolved and warmed. Handed
    //! back to the render thread, which uses it verbatim instead of re-drawing.
