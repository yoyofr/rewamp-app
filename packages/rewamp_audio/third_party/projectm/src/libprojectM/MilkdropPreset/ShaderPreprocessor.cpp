...
std::string ShaderPreprocessor::preprocess(const std::string& shaderSource) {
    ...
    // Step 0b: D3D9 fxc leaks a for-loop variable into the enclosing scope, and
    // presets use that to break an outer loop from inside an inner one.
    result = hoistLeakedLoopVariables(result);
    ...
    if (!shadowingCases.empty()) {
        ...
        for (const auto& shadow : shadowingCases) {
            ...
            while (pos < result.length()) {
                ...
                // A name that follows a '.' is a MEMBER or a swizzle, never the
                // shadowed variable - and a '.' passes the word-boundary test
                // above. Renaming it turned "uv.y += 0.05 * y;" into
                // "_uv.xy.y_float += 0.05 * y_float;", i.e. an invalid swizzle,
                // for every single-letter variable named x/y/z/w/r/g/b/a.
                bool afterDot = false;
                for (size_t back = pos; back > 0; --back) {
                    const char ch = result[back - 1];
                    if (ch == ' ' || ch == '\t' || ch == '\n' || ch == '\r') continue;
                    afterDot = (ch == '.');
                    break;
                }

                if (isWordStart && isWordEnd && !afterDot) {
...
//    }
    ...
    if (m_language == ShaderLanguage::HLSL) {
        // Names that are free in HLSL but RESERVED in GLSL, so a preset using
        // one as a variable ("float smooth, BPMtime, mh, zoom, …") produced a
        // shader the driver rejected with a bare "'smooth' : syntax error" -
        // and the transpilation stage saw nothing wrong at all. GLSL ES 3.0
        // interpolation/storage qualifiers and the reserved words most likely
        // to read as an ordinary variable name in a visualizer preset.
        std::vector<std::string> hlslKeywords = {
            "sample",
            "smooth", "flat", "noperspective",
            "input", "output", "filter", "active",
            "invariant", "precise", "layout", "patch",
        };
    ...
    while (foundComplexLoop) {
        ...
        for (const auto& loop : complexLoops) {
            ...
            // The PARSER handles the comma operator in both clauses now
            // (HLSLParser::ParseCommaSeparatedExpressions), so this rewrite has
            // nothing left to do - and it was actively harmful: on a for header
            // spread over several lines with comments in between it hoisted the
            // init out, replaced it with "1.0" and lost the ';', producing
            // "{ float2 r = uv.xy" then "for ( 1.0;++i < 79.".
            if (false && (hasMultipleInit || hasMultipleIncrement)) {
                loopToProcess = &loop;
            ...
            if (pos + 7 <= source.length() && source.substr(pos, 7) == "#define") {
                ...
                if (pos < source.length() && (source[pos] == ' ' || source[pos] == '\t')) {
                    ...
                    if (pos > nameStart) {
                        ...
                        // A trailing comment is NOT part of the value. Presets write
                        // "#define INV_MAX_ITER 0.2  // 1.0/MAX_ITER" all the time, and
                        // carrying the comment into the value comments out the rest of
                        // every line the macro is expanded into - the statement then
                        // loses its semicolon and the whole shader fails to parse.
                        const size_t lineComment = macroValue.find("//");
                        if (lineComment != std::string::npos) {
                            macroValue.erase(lineComment);
                        }
                        size_t blockComment = macroValue.find("/*");
                        while (blockComment != std::string::npos) {
                            const size_t blockEnd = macroValue.find("*/", blockComment + 2);
                            if (blockEnd == std::string::npos) {
                                macroValue.erase(blockComment);
                                break;
                            }
                            macroValue.erase(blockComment, blockEnd + 2 - blockComment);
                            blockComment = macroValue.find("/*", blockComment);
                        }
...
void ShaderPreprocessor::normalizeSamplerNames(std::string& program)
{
    // Milkdrop lets a preset declare a texture sampler under the bare texture
    // name - "sampler MilkDrop3_001;" then "tex2D(MilkDrop3_001, uv)" - while
    // everything downstream here keys on the "sampler_" prefix: the reference
    // scan, the descriptor names, the declarations re-inserted before parsing.
    // Rewriting the name up front is what lets the rest stay in one convention.
    auto isIdentifierChar = [](char c) {
        return std::isalnum(static_cast<unsigned char>(c)) != 0 || c == '_';
    };

    // Scan a comment-free copy, positions unchanged. Without this, a line like
    // "//2. High quality noise sampler" followed by "float3 hinoise = …" reads
    // as a declaration of a sampler called "float3", and renaming THAT breaks
    // every shader that mentions the type.
    std::string scan = program;
    for (size_t i = 0; i + 1 < scan.length(); i++)
    {
        if (scan[i] == '/' && scan[i + 1] == '/')
        {
        ...
        {
            const size_t end = scan.find("*/", i + 2);
            const size_t stop = (end == std::string::npos) ? scan.length() : end + 2;
            while (i < stop)
            {
                if (scan[i] != '\n')
                {
    ...
    size_t pos = 0;
    while ((pos = scan.find("sampler", pos)) != std::string::npos)
    {
        const size_t keywordStart = pos;
        pos += 7;
        // Must be the "sampler" keyword itself, not the tail of an identifier.
        if (keywordStart > 0 && isIdentifierChar(scan[keywordStart - 1]))
        {
            continue;
        }
        // Optional dimension suffix.
        if (scan.compare(pos, 2, "2D") == 0 || scan.compare(pos, 2, "3D") == 0)
        {
            pos += 2;
        }
        if (pos >= scan.length() || !std::isspace(static_cast<unsigned char>(scan[pos])))
        {
            continue;
        }
        while (pos < scan.length() && std::isspace(static_cast<unsigned char>(scan[pos])))
        {
            pos++;
        }
        const size_t nameStart = pos;
        while (pos < scan.length() && isIdentifierChar(scan[pos]))
        {
            pos++;
        }
        if (pos == nameStart)
        ...
        // A declaration is "sampler NAME;" - or a comma-separated declarator
        // LIST, "sampler rand01, rand02, martin_skin_x;", which MilkDrop accepts
        // and presets in the wild use. Only the first name was read, and the
        // ';' test below then rejected the whole line, so every sampler of such
        // a list stayed unprefixed and the shader died on an undeclared
        // identifier. Read the full list first; anything that is not terminated
        // by ';' ("sampler" as the last word of a sentence, a sampler_state
        // assignment, a function argument) is not a declaration, and renaming
        // on a loose match is how a type name once got rewritten across a whole
        // shader.
        std::vector<std::string> declared;
        size_t probe = nameStart;
        bool isDeclaration = false;
        while (true)
        {
            const size_t start = probe;
            while (probe < scan.length() && isIdentifierChar(scan[probe]))
            {
                probe++;
            }
            if (probe == start)
            ...
            declared.push_back(scan.substr(start, probe - start));
            while (probe < scan.length() && std::isspace(static_cast<unsigned char>(scan[probe])))
            {
                probe++;
            }
            if (probe < scan.length() && scan[probe] == ',')
            {
                probe++;
                while (probe < scan.length() && std::isspace(static_cast<unsigned char>(scan[probe])))
                {
            ...
            isDeclaration = (probe < scan.length() && scan[probe] == ';');
            break;
        }
        if (!isDeclaration)
        {
        ...
        {
            // Already prefixed, or the "sampler_state" block that is not a sampler.
            if (name.rfind("sampler_", 0) == 0 || name == "state")
            {
                continue;
            }
            // A name that is also a macro ("#define iChannel0 sampler_stoy_london"
            // next to "sampler iChannel0;") needs no rename: the macro replaces it
            // with a real sampler. Renaming it only invents a texture name nobody
            // ships, which the texture manager then reports as missing.
            const std::string define = "#define " + name;
            const size_t at = scan.find(define);
            if (at != std::string::npos &&
                (at + define.length() >= scan.length() ||
                 !isIdentifierChar(scan[at + define.length()])))
            {
                continue;
            }
            if (std::find(renamed.begin(), renamed.end(), name) == renamed.end())
            {
                renamed.push_back(name);
            }
        }
    }

    for (const auto& name : renamed)
    {
        // Whole-word replacement only: a bare name like "noise" would otherwise
        // also hit "noise_lq" and every local variable containing it.
        const std::string prefixed = "sampler_" + name;
        size_t at = 0;
        while ((at = program.find(name, at)) != std::string::npos)
        {
            const bool wordStart = (at == 0) || !isIdentifierChar(program[at - 1]);
            const size_t end = at + name.length();
            const bool wordEnd = (end >= program.length()) || !isIdentifierChar(program[end]);
            if (wordStart && wordEnd)
            {
                program.replace(at, name.length(), prefixed);
                at += prefixed.length();
            }
            else
            {
...
std::string ShaderPreprocessor::processDefines(const std::string& shaderSource) {
    ...
    if (defines.empty()) {
        return stripUndefDirectives(shaderSource);
    }
    ...
    // A macro with no value exists only to be tested with #ifdef/#ifndef, and
    // both of the things done below are wrong for it: substituting it deletes
    // the name from the very directive that tests it (leaving a bare "#ifdef",
    // which the HLSL parser rejects with "not an integer"), and removing its
    // definition means nothing downstream knows it was ever defined. Leave
    // those alone entirely - the HLSL parser's own preprocessor handles them.
    auto isFlagOnly = [](const DefineInfo& define) {
        return define.macroValue.empty() && !define.isFunctionLike;
    };

    // Is this occurrence sitting on a "#if…" line? Such a name is a name, not a
    // value, and must survive substitution. Resolved at match time by walking
    // back to the start of the line, because the text keeps shifting under us as
    // directives are removed and macros expanded.
    auto onConditionalLine = [](const std::string& text, size_t offset) {
        size_t lineStart = text.rfind('\n', offset);
        lineStart = (lineStart == std::string::npos) ? 0 : lineStart + 1;
        while (lineStart < offset && (text[lineStart] == ' ' || text[lineStart] == '\t')) {
            ++lineStart;
        }
        return text.compare(lineStart, 3, "#if") == 0;
    };
    ...
    for (auto it = defines.rbegin(); it != defines.rend(); ++it) {
        ...
        // A macro whose own VALUE names itself must never be re-scanned: C
        // forbids recursive expansion, and rescanning would loop (or, for
        // "#define A(x) A(A(x))", blow up). Decided once, per define, so the
        // nested-argument rescan below stays unconditionally safe.
        bool selfReferential = false;
        for (size_t sp = define.macroValue.find(define.macroName);
             sp != std::string::npos;
             sp = define.macroValue.find(define.macroName, sp + 1)) {
            const bool wordStart = (sp == 0 ||
                (!std::isalnum(static_cast<unsigned char>(define.macroValue[sp - 1])) &&
                 define.macroValue[sp - 1] != '_'));
            const size_t after = sp + define.macroName.length();
            const bool wordEnd = (after >= define.macroValue.length() ||
                (!std::isalnum(static_cast<unsigned char>(define.macroValue[after])) &&
                 define.macroValue[after] != '_'));
            if (wordStart && wordEnd) {
        ...
        while (searchPos < result.length()) {
            ...
            if (isWordStart && isWordEnd && onConditionalLine(result, foundPos)) {
                // "#if defined(FOO)" / "#ifdef FOO": the name is the subject of
                // the test, never a value to expand.
            ...
            if (isWordStart && isWordEnd) {
                ...
                if (define.isFunctionLike) {
                    ...
                    // Resume INSIDE the expansion. An argument can hold another
                    // call to the SAME macro -
                    //   GetPixel(uv + texsize.zw*GetPixel(uv).x)
                    // - and jumping past the whole expanded text left that inner
                    // call standing for ever, so the shader failed with
                    // "Undeclared identifier 'GetPixel'". Terminates because the
                    // name can only reappear through an argument (self-
                    // referential values are excluded above) and each pass
                    // consumes one more level of nesting. 39 presets of
                    // cream-of-the-crop failed on exactly this.
                    searchPos = selfReferential ? foundPos + expandedValue.length()
                                                : foundPos;
                } else {
    ...
    return stripUndefDirectives(result);
}

std::string ShaderPreprocessor::hoistLeakedLoopVariables(const std::string& shaderSource) {
    static const std::vector<std::string> kTypes = {
        "float4", "float3", "float2", "float", "int4", "int3", "int2", "int", "uint", "bool"};

    auto isIdentifierChar = [](char c) {
        return std::isalnum(static_cast<unsigned char>(c)) != 0 || c == '_';
    };
    auto isWholeWordAt = [&](const std::string& text, size_t at, const std::string& word) {
        const bool startOk = (at == 0) || !isIdentifierChar(text[at - 1]);
        const size_t end = at + word.length();
        const bool endOk = (end >= text.length()) || !isIdentifierChar(text[end]);
    ...
    while (true) {
        const size_t forPos = result.find("for", searchFrom);
        if (forPos == std::string::npos) {
            break;
        }
        searchFrom = forPos + 3;
        if (!isWholeWordAt(result, forPos, "for")) {
            continue;
        }
        ...
        const size_t parenPos = pos;
        ++pos;
        while (pos < result.length() && std::isspace(static_cast<unsigned char>(result[pos]))) {
            ++pos;
        }

        // The init clause must declare a variable of a known scalar/vector type.
        std::string type;
        for (const auto& candidate : kTypes) {
            if (result.compare(pos, candidate.length(), candidate) == 0 &&
                !isIdentifierChar(result[pos + candidate.length()])) {
                type = candidate;
                break;
            }
        }
        ...
        size_t namePos = pos + type.length();
        while (namePos < result.length() && std::isspace(static_cast<unsigned char>(result[namePos]))) {
            ++namePos;
        }
        size_t nameEnd = namePos;
        while (nameEnd < result.length() && isIdentifierChar(result[nameEnd])) {
            ++nameEnd;
        }
        if (nameEnd == namePos) {
            continue;
        }
        const std::string name = result.substr(namePos, nameEnd - namePos);

        // Find the loop body and the end of the block that contains the loop.
        const size_t bodyStart = result.find('{', nameEnd);
        if (bodyStart == std::string::npos) {
            continue;
        }
        // Local brace matcher: the member one is non-static and this pass has no
        // instance state to speak of.
        size_t bodyEnd = std::string::npos;
        {
            int open = 0;
            for (size_t i = bodyStart; i < result.length(); ++i) {
                if (result[i] == '{') {
                    ++open;
                } else if (result[i] == '}') {
                    if (--open == 0) {
        ...
        int depth = 0;
        size_t enclosingEnd = result.length();
        for (size_t i = bodyEnd + 1; i < result.length(); ++i) {
            if (result[i] == '{') {
                ++depth;
            } else if (result[i] == '}') {
                if (depth == 0) {
        ...
        // Only leak it if something after the loop actually reads or writes it.
        bool usedAfter = false;
        for (size_t at = result.find(name, bodyEnd + 1);
             at != std::string::npos && at < enclosingEnd;
             at = result.find(name, at + 1)) {
            if (isWholeWordAt(result, at, name)) {
                usedAfter = true;
                break;
            }
        }
        ...
        // "for(float j = 1.; …)" -> "float j; for(j = 1.; …)".
        const std::string declaration = type + " " + name + "; ";
        result.erase(parenPos + 1, type.length() + (namePos - (pos + type.length())));
        result.insert(forPos, declaration);
        searchFrom = forPos + declaration.length() + 3;
    }

    return result;
}

std::string ShaderPreprocessor::stripUndefDirectives(const std::string& shaderSource) {
    // "#undef X" then "#define X <other value>" is how presets redefine a macro.
    // Substitution above already gives the last definition priority (it runs the
    // definitions in reverse, so the latest one consumes every occurrence), which
    // leaves the directive itself with nothing to do - and the HLSL parser has no
    // #undef at all, so leaving it in is a syntax error.
    std::string result;
    result.reserve(shaderSource.length());
    ...
    while (lineStart < shaderSource.length()) {
        ...
        if (lastLine) {
            lineEnd = shaderSource.length();
        }

        size_t firstChar = lineStart;
        while (firstChar < lineEnd &&
               (shaderSource[firstChar] == ' ' || shaderSource[firstChar] == '\t')) {
            ++firstChar;
        }

        if (shaderSource.compare(firstChar, 6, "#undef") != 0) {
            result.append(shaderSource, lineStart, lineEnd - lineStart);
        }
        if (!lastLine) {
            result.push_back('\n');   // keep the line count, error messages point at it
        }
        lineStart = lineEnd + 1;
    }
