...
public:
     ...
     * Milkdrop accepts "sampler MyTexture;" with matching uses, while the rest of
     * the pipeline keys on that prefix (reference scan, descriptor names, the
     * declarations re-inserted before parsing). Static so the offline oracle can
     * run the very same code instead of a copy that drifts.
     */
    static void normalizeSamplerNames(std::string& program);

    /**
     * @brief Removes "#undef" lines, which the HLSL parser does not know.
     * Macro substitution already gives the LAST definition of a name priority,
     * so the directive has nothing left to do by the time it is reached.
     */
    static std::string stripUndefDirectives(const std::string& shaderSource);

    /**
     * @brief Hoists a for-loop variable out of the loop when it is used after it.
     * D3D9's fxc leaks the loop variable into the enclosing scope, and presets
     * written against it do "for(float j…){…} … j = 5;" to break an outer loop.
     * Only hoisted where such a later use exists, so ordinary loops keep their
     * own scope and two sibling loops can still both declare "i".
     */
    static std::string hoistLeakedLoopVariables(const std::string& shaderSource);
