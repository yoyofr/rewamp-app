// FAST_RENDER draws a custom shape in a single call, doing border+texture+
// additive/alpha blending inside one shader via GL_EXT_shader_framebuffer_fetch.
// That extension is available on kakashidinho MetalANGLE (iOS) but NOT on
// upstream ANGLE's Metal backend (macOS) — build with -DCUSTOMSHAPE_NO_FAST_RENDER
// there to fall back to the standard (upstream projectM) multi-draw blend path.
// rewamp: the fast path is now selected at RUNTIME (FramebufferFetch.hpp) —
// whether GL_EXT_/GL_ARM_shader_framebuffer_fetch exists is a per-GPU/driver
// fact, not a build-time one. CUSTOMSHAPE_NO_FAST_RENDER still forces it off.
    ...
    , m_perFrameContext(presetState.globalMemory, &presetState.globalRegisters)
{
    ...
    if (MilkdropPreset::FramebufferFetchAvailable()) {
...
void CustomShape::Draw()
{
    ...
    for (int instance = 0; instance < m_instances; instance++)
    {
        ...
        for (int i = 1; i < sides + 1; i++)
        {
            ...
            // borderFactor, NOT 1.0. The fragment shader draws the border where
            // the interpolated flag reaches 1.0; with 1.0 at the corners that is
            // only the outer EDGE - a line of zero width, which rounding erases
            // entirely. Writing borderFactor = (rad+thickAdd)/rad makes the
            // interpolation cross 1.0 exactly at radius `rad`, so the band
            // between rad and rad+thickAdd IS the border, which is what the
            // factor was computed for (it was dead code). A preset whose fill is
            // black and whose only visible content is the outline - "Flexi -
            // lorenz attractor" - rendered a fully black screen.
            m_borderFlag[vertexIdx+i].borderFlag = borderFactor;
        }
        ...
        m_borderFlag[vertexIdx + sides + 1].borderFlag = borderFactor;
