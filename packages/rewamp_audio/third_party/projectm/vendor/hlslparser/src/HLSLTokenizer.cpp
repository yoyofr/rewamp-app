...
void HLSLTokenizer::Next(const bool EOLSkipping)
{
    ...
    // "# define X" is the same directive as "#define X" - C allows blanks
    // between the '#' and the directive name, and presets in the wild use it.
    // The scan above stops at the space, leaving a lone "#" that reaches the
    // parser as an identifier ("expected ';' near '#'"). Glue the next word on,
    // but only across blanks on the SAME line: a '#' alone on its line is the
    // null directive and must stay one.
    if (length == 1 && m_identifier[0] == '#')
    {
        const char* probe = m_buffer;
        while (probe < m_bufferEnd && (probe[0] == ' ' || probe[0] == '\t'))
        {
            ++probe;
        }
        const char* wordStart = probe;
        while (probe < m_bufferEnd && probe[0] != 0 && !GetIsSymbol(probe[0]) && !isspace(probe[0]))
        {
            ++probe;
        }
        const size_t wordLength = probe - wordStart;
        if (wordLength > 0 && wordLength + 1 < s_maxIdentifier)
        {
            memcpy(m_identifier + 1, wordStart, wordLength);
            m_identifier[wordLength + 1] = 0;
            const int numWords = sizeof(_reservedWords) / sizeof(const char*);
            bool matched = false;
            for (int i = 0; i < numWords && !matched; ++i)
            {
                matched = (strcmp(_reservedWords[i], m_identifier) == 0);
            }
            if (matched)
            {
                m_buffer = probe;
            ...
            {
                m_identifier[1] = 0;   // not a directive: leave the '#' alone
            }
        }
    }
