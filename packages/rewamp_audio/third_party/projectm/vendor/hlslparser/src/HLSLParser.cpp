...
const Intrinsic _intrinsic[] =
    {
        ...
        // HLSL has both, and Shadertoy ports lean on them; the GLSL generator
        // passes the names through unchanged since GLSL spells them the same.
...
bool HLSLParser::ParseStatement(HLSLStatement*& statement, const HLSLType& returnType)
{
    ...
    if (Accept(HLSLToken_For))
    {
        ...
        // Comma-separated init clause ("for (o *= i, p = q; ...)"). Chained on
        // nextExpression, which the generator emits comma-separated - the same
        // shape an argument list already uses.
        if (forStatement->initialization == NULL)
        {
            ParseCommaSeparatedExpressions(forStatement->initializationWithoutType);
        }
        if (!Expect(';'))
        {
            return false;
        ...
        // Comma operator in the increment clause. Shadertoy-derived presets put
        // the whole loop body there:
        //   for (…; ++i < 79.; z += d + 5E-4, q = float4(…), q.z -= T/5, …)
        // Without this the parser stopped at the first comma and the heuristic
        // rewriter upstream had to hoist the clause out of the header - which is
        // what mangled those loops.
        if (!ParseCommaSeparatedExpressions(forStatement->increment))
        {
            return false;
        }
    ...
    {
        // Comma operator: "u += a, u += b;" is ONE statement. The fallback just
        // below treats a comma as a statement SEPARATOR, which is fine inside a
        // block but wrong as an unbraced loop body - only the first expression
        // became the body and the rest landed AFTER the loop, outside the for's
        // scope ("Undeclared identifier 'i'"). Shadertoy-derived presets write
        // whole loop bodies that way.
        if (!ParseCommaSeparatedExpressions(expression))
        {
            return false;
        }
...
// Appends further comma-separated expressions onto [expression] via
// nextExpression. Used for the init and increment clauses of a for statement,
// where C's comma operator is legal and common in shadertoy-derived presets.
// A NULL expression (empty clause) is left alone.
bool HLSLParser::ParseCommaSeparatedExpressions(HLSLExpression*& expression)
{
    if (expression == NULL)
    {
        return true;
    }
    HLSLExpression* last = expression;
    while (last->nextExpression != NULL)
    {
        last = last->nextExpression;
    }
    while (Accept(','))
    {
        HLSLExpression* next = NULL;
        if (!ParseExpression(next) || next == NULL)
        {
            return false;
        }
        last->nextExpression = next;
        while (last->nextExpression != NULL)
        {
            last = last->nextExpression;
        }
...
bool HLSLParser::ParseBinaryExpression(int priority, HLSLExpression*& expression)
{
    ...
    while (1)
    {
        ...
        else if (_conditionalOpPriority > priority && Accept('?'))
        {
            ...
            HLSLExpression* expression2 = NULL;
            // Both branches are parsed at priority 0, NOT at the conditional's
            // own priority: the acceptance test above is
            // "_conditionalOpPriority > priority", so re-entering at that same
            // priority forbade a NESTED ternary and
            //   c<.666 ? c<.333 ? h : h+1. : h+step(f.yx, f)
            // died on "expected ':' near '?'". C allows a full expression
            // between '?' and ':', and a conditional-expression after ':'
            // (right associative) - priority 0 is exactly both.
            if (!ParseBinaryExpression(0, expression1) || !Expect(':') || !ParseBinaryExpression(0, expression2))
            {
                return false;
            }
        ...
        else
        {
            ...
            if( needsExpressionEndChar != 0 )
            {
                // The postfix chain can sit on EITHER side of that pending
                // parenthesis, and only the terminal parser ran it - for a
                // constructor it returned before the ')' was even consumed:
                //   "(float4(...).x)"  the '.' comes FIRST, so run postfix now
                //                      and let the ')' be consumed next round;
                //   "(float3(...)).xy" the ')' comes first, handled below.
                // Consuming the ')' unconditionally answered the first form with
                // "expected ')' near '.'", and skipping the second left the '.'
                // to the statement parser ("expected ';' near '.'").
                const int pending = m_tokenizer.GetToken();
                if (pending == '.' || pending == '[')
                {
                    if (!ParsePostfixOperators(expression, fileName, line))
                        return false;
                    acceptBinaryOp = false;
                    continue;
                }
                ...
                const int next = m_tokenizer.GetToken();
                if (next == '.' || next == '[')
                {
                    if (!ParsePostfixOperators(expression, fileName, line))
                        return false;
        ...
        if( needsExpressionEndChar != 0 )
        {
            // Same two forms as above: postfix before the pending ')' or after.
            const int pending = m_tokenizer.GetToken();
            if (pending == '.' || pending == '[')
            {
                if (!ParsePostfixOperators(expression, fileName, line))
...
bool HLSLParser::ParseTerminalExpression(HLSLExpression*& expression, char& needsExpressionEndChar)
{
    ...
    return ParsePostfixOperators(expression, fileName, line);
}

// The postfix chain (++/--, .swizzle, [index], call). Split out of
// ParseTerminalExpression so the caller can run it again after it has consumed a
// deferred closing parenthesis: "(float3(a,b,c)).xy" returns from the partial
// constructor with the ')' still owed to ParseBinaryExpression, which used to
// swallow it and then find a '.' nobody would parse ("expected ';' near '.'").
bool HLSLParser::ParsePostfixOperators(HLSLExpression*& expression, const char* fileName, int line)
{
    bool done = false;
    while (!done)
    {
...
bool HLSLParser::ParsePreprocessorDefine()
{
    ...
    // Prepare next token. WITHOUT end-of-line skipping: a value-less
    // "#define FEATURE" otherwise stepped straight over its own newline onto the
    // next line, and the value loop below - which stops at the first
    // end-of-line - then ate that whole line as the macro's value. The line was
    // consumed AND dropped from the output, so a second bare "#define OTHER"
    // right after one was never registered at all and every "#ifdef OTHER"
    // block silently vanished (measured on BigWings "Heartfelt": the entire
    // USE_POST_PROCESSING body went missing, and the shader then failed on the
    // 'fade' its dropped declaration defined).
...
bool HLSLParser::ApplyPreprocessor(const char* fileName, const char* buffer, size_t length,
    ...
    while (m_tokenizer.GetToken() != HLSLToken_EndOfStream && !isCodeActive.empty())
    {
        ...
        {
            // "#if 1", "#if FEATURE" (a macro whose value is a number), and
            // "#if defined(FEATURE)". A bare identifier has to resolve here
            // because the earlier passes deliberately leave names alone on
            // conditional lines - substituting them is what turns "#ifdef X"
            // into a bare "#ifdef".
            bool active = true;
            bool decided = false;
            bool expectDefined = false;

            while (m_tokenizer.GetToken() != HLSLToken_EndOfLine &&
                   m_tokenizer.GetToken() != HLSLToken_EndOfStream)
            {
                if (m_tokenizer.GetToken() == HLSLToken_IntLiteral && !decided)
                {
                    active = m_tokenizer.GetInt() != 0;
                    decided = true;
                }
                else if (m_tokenizer.GetToken() == HLSLToken_Identifier)
                {
                    const char* identifier = m_tokenizer.GetIdentifier();
                    if (String_Equal(identifier, "defined"))
                    {
                        expectDefined = true;
                    }
                    else if (!decided)
                    {
                        const HLSLMacro* macro = NULL;
                        for (int i = m_macros.GetSize() - 1; i >= 0; --i)
                        {
                            if (String_Equal(m_macros[i]->name, identifier))
                            {
                                macro = m_macros[i];
                                break;
                            }
                        ...
                        {
                            active = false;   // undefined names are 0, as in C
                        }
                        else
                        {
                            // A value that is not a plain number (an expression,
                            // or nothing at all) counts as defined-and-true.
                            const char* value = macro->value;
                            char* end = NULL;
                            const long number = value ? strtol(value, &end, 0) : 0;
                            active = (value && end && end != value && *end == '\0')
                                         ? (number != 0)
                                         : true;
                        }
                        decided = true;
            ...
            // Nothing recognisable: keep the code rather than lose the shader.
            isCodeActive.push(active);
            addOriginalSource = false;
        }
...
const HLSLFunction* HLSLParser::MatchFunctionCall(const HLSLFunctionCall* functionCall, const char* name)
{
    ...
    bool matchedIsUserFunction  = false;
    ...
    for (int i = 0; i < _numIntrinsics; ++i)
    {
        ...
        if (String_Equal(function->name, name))
        {
            ...
            if (result == Function1Better)
            {
                matchedFunction = function;
                numMatchedOverloads = 1;
                matchedIsUserFunction = false;
            }
            else if (result == FunctionsEqual)
            {
                // A preset that DEFINES a function of its own wins the tie
                // against the intrinsic of the same signature - shadertoy ports
                // ship their own "float saturate(float x) { return clamp(...); }"
                // and the tie was reported as an ambiguity, killing the shader.
                // Only an intrinsic that is strictly BETTER (above) still wins.
                if (!matchedIsUserFunction)
                {
                    ++numMatchedOverloads;
                }
            }
