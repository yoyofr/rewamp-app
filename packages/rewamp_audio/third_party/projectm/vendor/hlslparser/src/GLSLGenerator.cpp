...
const char* GLSLGenerator::s_reservedWord[] =
    {
		...
		"main",
        "sample",
        // GLSL builtins that HLSL does NOT have, so preset authors porting
        // Shadertoy code declare them themselves ("float4 lessThan(float4 a,
        // float4 b) {…}"). GLSL refuses the redeclaration; renaming the user's
        // version keeps it, and the generator's own calls to the real builtins
        // are written literally, not through GetSafeIdentifierName.
        // Deliberately limited to names HLSL does NOT define: renaming a name
        // that IS an HLSL intrinsic (any, all, …) would rewrite calls meant for
        // the intrinsic into calls to a function nobody declared.
        "lessThan",
        "greaterThan",
        "lessThanEqual",
        "greaterThanEqual",
...
// True for a divisor that is a literal other than zero. Dividing by one cannot
// diverge from DX9 - the zero rule never fires - so such a division must stay a
// plain "/" rather than become a call. Two reasons, and the first is not an
// optimisation: a GLSL "const" initializer may not contain a function call, so
// routing "const float4 s[5] = {float4(0,0,0,11/3), ...}" through the helper
// makes the driver reject the whole shader ("assigning non-constant to const").
static bool IsNonZeroLiteral(const HLSLExpression* expression)
{
    if (expression == NULL || expression->nodeType != HLSLNodeType_LiteralExpression) return false;
    const HLSLLiteralExpression* literal = static_cast<const HLSLLiteralExpression*>(expression);
    switch (literal->type)
    {
    case HLSLBaseType_Float:  return literal->fValue != 0.0f;
    case HLSLBaseType_Int:
    case HLSLBaseType_Uint:   return literal->iValue != 0;
    default:                  return false;
    }
}

// True for an lvalue that can be written TWICE with no cost and no change of
// meaning: a plain name, a field or swizzle of one, an array element at a
// literal index. "a *= b" can only be rewritten as "a = mult0(a, b)" when that
// holds - anything else (a computed index, a call) would be evaluated a second
// time. Measured on the 11 585 presets of the five packs: every single compound
// assignment qualifies, the one array target in the whole corpus being
// "xpos[1]". So the guard costs nothing in coverage and removes the risk
// outright rather than betting the corpus never grows a nastier case.
static bool IsSimpleLValue(const HLSLExpression* expression)
{
    ...
    case HLSLNodeType_MemberAccess:
        return IsSimpleLValue(static_cast<const HLSLMemberAccess*>(expression)->object);
    case HLSLNodeType_ArrayAccess:
    {
        const HLSLArrayAccess* access = static_cast<const HLSLArrayAccess*>(expression);
        return access->index != NULL &&
               access->index->nodeType == HLSLNodeType_LiteralExpression &&
               IsSimpleLValue(access->array);
    }
    default:
        return false;
    }
...
bool GLSLGenerator::Generate(HLSLTree* tree, Target target, Version version, const char* entryName, const Options& options)
{
    ...
    ChooseUniqueName( "div", m_altDivFunction, sizeof( m_altDivFunction ) );
    ...
    if (m_options.flags & Flag_AlternateNanPropagation) {
        ...
        // Division needs the SAME rule, and it is not a second special case: a
        // DX9 pixel shader has no divide instruction, "x / y" compiles to
        // rcp(y) followed by mul, so the zero rule above is what a preset
        // author actually observed. rcp(0) is INF there too, but 0 * INF then
        // yields 0 instead of the NaN IEEE gives us - and a NaN does not stay
        // local, it is added into the pixel and blackens the WHOLE screen.
        // Paid on "MilkDrop2077 - 02_MW_new01B": q22..q25 are never written by
        // the preset, so a rotation matrix built from them is all zeros, the
        // vector it rotates becomes exactly 0, the length in the denominator
        // with it, and "3*q22/cross" is a plain 0/0 that every frame turned
        // black.
        m_writer.WriteLine(0, "float %s(int i_x, int i_y) { float x=float(i_x); if (x == 0.0) { return 0.0; } else { return (x / float(i_y)); } }", m_altDivFunction);
        m_writer.WriteLine(0, "float %s(int i_x, float y) { float x=float(i_x); if (x == 0.0) { return 0.0; } else { return (x / y); } }", m_altDivFunction);
        m_writer.WriteLine(0, "float %s(float x, int i_y) { if (x == 0.0) { return 0.0; } else { return (x / float(i_y)); } }", m_altDivFunction);
        m_writer.WriteLine(0, "float %s(float x, float y) { if (x == 0.0) { return 0.0; } else { return (x / y); } }", m_altDivFunction);
        m_writer.WriteLine(0, "vec2 %s(vec2 x, vec2 y) { return vec2(%s(x.x, y.x), %s(x.y, y.y)); }", m_altDivFunction, m_altDivFunction, m_altDivFunction);
        m_writer.WriteLine(0, "vec3 %s(vec3 x, vec3 y) { return vec3(%s(x.x, y.x), %s(x.y, y.y), %s(x.z, y.z)); }", m_altDivFunction, m_altDivFunction, m_altDivFunction, m_altDivFunction);
        m_writer.WriteLine(0, "vec3 %s(vec3 x, float y) { return %s(x, vec3(y, y, y)); }", m_altDivFunction, m_altDivFunction);
        m_writer.WriteLine(0, "vec3 %s(float x, vec3 y) { return %s(vec3(x, x, x), y); }", m_altDivFunction, m_altDivFunction);
        m_writer.WriteLine(0, "ivec3 %s(ivec3 x, ivec3 y) { return ivec3(%s(float(x.x), float(y.x)), %s(float(x.y), float(y.y)), %s(float(x.z), float(y.z))); }", m_altDivFunction, m_altDivFunction, m_altDivFunction, m_altDivFunction);
        m_writer.WriteLine(0, "vec4 %s(vec4 x, vec4 y) { return vec4(%s(x.x, y.x), %s(x.y, y.y), %s(x.z, y.z), %s(x.w, y.w)); }", m_altDivFunction, m_altDivFunction, m_altDivFunction, m_altDivFunction, m_altDivFunction);
    }
...
//                m_writer.Write(")))");
                ...
                // GLSL has no integer mod(), so the operands were floated - but
                // that makes the RESULT a float while HLSL says "int % int" is
                // an int, and everything downstream trusts the HLSL type. The
                // assignment "int c; c = i % 3;" then came out as
                // "c = mod(float(i), float(3))" and the driver refused it
                // ("cannot convert from 'mediump float' to 'mediump int'"). Cast
                // the result back when the HLSL type is integral - and only
                // then, which is why the old unconditional int() cast, still
                // commented out above, could not stay.
                const HLSLBaseType modResult = binaryExpression->expressionType.baseType;
                const bool intResult =
                    (modResult == HLSLBaseType_Int  || modResult == HLSLBaseType_Uint  ||
                     modResult == HLSLBaseType_Int2 || modResult == HLSLBaseType_Uint2 ||
                     modResult == HLSLBaseType_Int3 || modResult == HLSLBaseType_Uint3 ||
                     modResult == HLSLBaseType_Int4 || modResult == HLSLBaseType_Uint4);
                if (intResult) m_writer.Write("%s(", GetTypeName(binaryExpression->expressionType));
                m_writer.Write("(mod(");
                ...
                if (forceType) m_writer.Write(")");
                m_writer.Write("))");
                if (intResult) m_writer.Write(")");

            } else {
                bool handled = false;
                if (m_options.flags & Flag_AlternateNanPropagation) {
                    // Punt to functions that do not follow IEEE 754 NaN
                    // propagation rules. Division belongs here for the same
                    // reason multiplication does: DX9 has no divide, it emits
                    // rcp + mul, so the zero rule covers both.
                    const char* altFn = NULL;
                    bool compound = false;
                    if (binaryExpression->binaryOp == HLSLBinaryOp_Mul) altFn = m_altMultFunction;
                    else if (binaryExpression->binaryOp == HLSLBinaryOp_Div) altFn = m_altDivFunction;
                    // "a *= b" and "a /= b" carry the same hazard as "a * b" and
                    // "a / b" - the compound form is not a different operator,
                    // it just had no rewrite. It gets one, as "a = fn(a, b)",
                    // under two conditions that make it free of risk rather than
                    // merely likely to work: the right-hand side must not be a
                    // literal that rules the zero case out anyway, and the
                    // target must be duplicable (see IsSimpleLValue).
                    else if (binaryExpression->binaryOp == HLSLBinaryOp_MulAssign ||
                             binaryExpression->binaryOp == HLSLBinaryOp_DivAssign) {
                        // Floating-point targets only. The helpers all return a
                        // float, so "int col; col *= -1;" would come back as
                        // "col = mult0(col, int(-1))" and the driver refuses it
                        // ("cannot convert from 'highp float' to 'mediump int'",
                        // 5 presets of the smooth-spectrum-analyser family). No
                        // loss either way: the zero rule exists to stop NaN and
                        // INF propagating, and an integer has neither.
                        const HLSLBaseType targetType = binaryExpression->expression1->expressionType.baseType;
                        const bool floatTarget =
                            (targetType == HLSLBaseType_Float  || targetType == HLSLBaseType_Float2 ||
                             targetType == HLSLBaseType_Float3 || targetType == HLSLBaseType_Float4);
                        if (floatTarget && IsSimpleLValue(binaryExpression->expression1)) {
                            altFn = (binaryExpression->binaryOp == HLSLBinaryOp_MulAssign)
                                        ? m_altMultFunction : m_altDivFunction;
                            compound = true;
                        }
                    }
                    // A literal divisor/factor other than zero can never make the
                    // rule fire, and wrapping it would put a function call where
                    // a GLSL "const" initializer forbids one.
                    if (altFn != NULL && binaryExpression->binaryOp != HLSLBinaryOp_Mul &&
                        IsNonZeroLiteral(binaryExpression->expression2)) altFn = NULL;
                    if (altFn != NULL) {
                        if (compound) {
                            OutputExpression(binaryExpression->expression1, NULL);
                            m_writer.Write(" = ");
                        }
                        m_writer.Write("%s(", altFn);
                        OutputExpression(binaryExpression->expression1, dstType1);
                        m_writer.Write(",");
                        OutputExpression(binaryExpression->expression2, dstType2);
                        m_writer.Write(")");
    ...
    else if (expression->nodeType == HLSLNodeType_ConditionalExpression)
    {
		...
		{
			// Both branches must land on the SAME type: HLSL happily mixes
			// "a > 0 ? a*w : 0." but GLSL ES rejects it with "mismatching
			// ternary operator operand types 'const int' and 'highp float'".
			// The vector branch above already casts to the conditional's own
			// type; this one only forwarded the caller's dstType, which is NULL
			// in a plain expression statement, so nothing was cast at all.
			const HLSLType* branchType =
				(dstType != NULL) ? dstType : &conditionalExpression->expressionType;
			m_writer.Write( "((" );
			OutputExpression( conditionalExpression->condition, &kBoolType );
			m_writer.Write( ")?(" );
			OutputExpression( conditionalExpression->trueExpression, branchType );
			m_writer.Write( "):(" );
			OutputExpression( conditionalExpression->falseExpression, branchType );
			m_writer.Write( "))" );
		}
    }
...
void GLSLGenerator::OutputStatements(int indent, HLSLStatement* statement, const HLSLType* returnType)
{
    ...
    while (statement != NULL)
    {
        ...
        else if (statement->nodeType == HLSLNodeType_Function)
        {
            ...
            // A body-less function is a prototype, whatever the parser flagged:
            // emitting "float f(…) { }" for one makes the GLSL compiler reject
            // it with "does not return a value", where a prototype it never has
            // to resolve (the preset declares it and never calls it) is legal.
            if (function->forward || function->statement == NULL)
            {
                m_writer.WriteLine(indent, ");");
            }
            else
        ...
        else if (statement->nodeType == HLSLNodeType_ExpressionStatement)
        {
            ...
            // Comma operator: the parser chains "a, b, c;" on nextExpression.
            // Emitting only the head would drop the rest in silence.
            OutputExpressionList(expressionStatement->expression);
            m_writer.EndLine(";");
        }
        ...
        else if (statement->nodeType == HLSLNodeType_ForStatement)
        {
            ...
            {
                // Comma operator: the parser chains the clause's expressions on
                // nextExpression, and GLSL accepts the same comma-separated
                // form in a for header.
                OutputExpressionList(forStatement->initializationWithoutType);
            }
            m_writer.Write("; ");
            OutputExpression(forStatement->condition, &kBoolType);
            m_writer.Write("; ");
            OutputExpressionList(forStatement->increment);
            m_writer.Write(") {");
            m_writer.EndLine();
...
// Whether an initializer is a constant expression in the GLSL sense.
// Deliberately conservative: anything it can't prove constant is treated as
// runtime, which only ever costs a dropped "const" qualifier.
static bool IsConstantInitializer(const HLSLExpression* expression)
{
	if (expression == NULL)
	{
		return true;
	...
	case HLSLNodeType_LiteralExpression:
		return true;
	case HLSLNodeType_UnaryExpression:
		return IsConstantInitializer(static_cast<const HLSLUnaryExpression*>(expression)->expression);
	case HLSLNodeType_BinaryExpression:
	{
		const HLSLBinaryExpression* binary = static_cast<const HLSLBinaryExpression*>(expression);
		return IsConstantInitializer(binary->expression1) && IsConstantInitializer(binary->expression2);
	}
	case HLSLNodeType_CastingExpression:
		return IsConstantInitializer(static_cast<const HLSLCastingExpression*>(expression)->expression);
	case HLSLNodeType_ConstructorExpression:
	{
		for (const HLSLExpression* argument = static_cast<const HLSLConstructorExpression*>(expression)->argument;
		     argument != NULL;
		     argument = argument->nextExpression)
		{
			if (!IsConstantInitializer(argument))
			{
				return false;
			}
		}
	...
	default:
		// Identifiers (even of const variables), function calls, member access,
		// texture fetches: not provably constant.
		return false;
	}
}
...
{
	// HLSL's "const" only means read-only, and presets lean on that:
	// "const float t = time * 0.25;" is ordinary HLSL. GLSL's const is stricter
	// - it demands a constant expression - so the qualifier has to go whenever
	// the initializer is a runtime value, or the shader fails to compile with
	// "assigning non-constant to 'const float'". Kept where the initializer IS
	// constant, because a const global is what makes an array size legal.
	HLSLType type = declaration->type;
	if ((type.flags & HLSLTypeFlag_Const) != 0)
	{
		for (HLSLDeclaration* d = declaration; d != NULL; d = d->nextDeclaration)
		{
			// A deferred (global) assignment lands after the declaration, which
			// no const can survive either.
			if (d->assignment != NULL && (skipAssignment || !IsConstantInitializer(d->assignment)))
			{
				type.flags &= ~HLSLTypeFlag_Const;
				break;
			}
		}
	}
	...
	while( declaration )
	{
		...
		else if (skipAssignment && !type.array &&
		         type.baseType >= HLSLBaseType_FirstNumeric &&
		         type.baseType <= HLSLBaseType_LastNumeric)
		{
			// Zero a global with no initializer. HLSL/D3D9 gives globals a value
			// of 0, GLSL leaves them undefined, and presets rely on the former -
			// often by accident, when the author comments an assignment out
			// ("//uv4 = …") and keeps reading the variable. Undefined there means
			// sampling a texture at a garbage coordinate, i.e. a nearly black
			// screen instead of the intended image.
...
void GLSLGenerator::OutputDeclarationAssignment(HLSLDeclaration* declaration)
{
   ...
   if( declaration->type.array )
   {
       ...
       // If base type is a vector and we have SCALAR assignments, group them
       // into constructors. The scalar test is load-bearing: a preset that
       // already writes the elements out - "float2 d[8] = {float2(-1,-1),
       // float2(-1,0), …}" - had its vec2s grouped two by two into
       // "vec2( vec2(…), vec2(…) )", which the driver answered with
       // "'constructor' : too many arguments".
       const bool scalarInitializers =
           declaration->assignment != NULL &&
           IsScalarType(declaration->assignment->expressionType);
       if (componentCount > 1 && scalarInitializers)
       {
           HLSLExpression* expr = declaration->assignment;
   ...
   else
   {
       ...
       else if (declaration->type.baseType == HLSLBaseType_UserDefined &&
                declaration->assignment->nextExpression == NULL &&
                declaration->assignment->expressionType.baseType == HLSLBaseType_UserDefined &&
                declaration->type.typeName != NULL &&
                declaration->assignment->expressionType.typeName != NULL &&
                String_Equal(declaration->type.typeName,
                             declaration->assignment->expressionType.typeName))
       {
           // A struct initialised from another struct of the SAME type is a
           // copy, not a constructor call: GLSL's struct constructor takes one
           // argument per FIELD, so wrapping it emitted
           // "Circle cir = Circle( circles[num] );" and the driver answered
           // "Number of constructor parameters does not match the number of
           // structure fields". Emit the initialiser bare.
