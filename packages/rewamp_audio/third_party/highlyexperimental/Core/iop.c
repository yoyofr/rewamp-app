...
** Auditing functions
*/
static EMU_INLINE void EMU_CALL iop_audit(struct IOP_STATE *state, uint32 address, uint32 length, uint8 t) {
...
//  uint32 i;
  ...
  */
  if(state->audit_map) {
    iop_audit(state, mem_address, len, is_writing ? IOP_AUDIT_READ : IOP_AUDIT_WRITE);
  }
