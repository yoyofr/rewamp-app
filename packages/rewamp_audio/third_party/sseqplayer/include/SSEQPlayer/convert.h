...
#include <typeinfo>
#include <locale>
/* rewamp: le #if d'origine disait « GCC/Clang sans libc++ » pour retomber sur
 * des polyfills copiés de libc++ (wstring_convert.h / codecvt.h). Ce test
 * datait de GCC 4.x, qui n'avait pas encore <codecvt>. Sur une libstdc++
 * moderne (Linux) il est FAUX et nuisible: les polyfills injectent leurs
 * propres std::wstring_convert / std::codecvt_utf8 dans namespace std, où la
 * bibliothèque les définit déjà — d'où « reference to 'wstring_convert' is
 * ambiguous ». <codecvt> existe partout depuis C++11 (déprécié depuis C++17,
 * mais toujours livré par libc++ comme par libstdc++), on le prend donc
 * inconditionnellement. Apple n'est pas affecté: il prenait déjà cette
 * branche-là. */
#include <codecvt>
#include <vector>
#include <cmath>
