...
int rewamp_mw_active = 1;   /* rewamp: STE DMA (voice 3) audible flag */
#endif
...
static void rescale(s32 * b, int f, int n)
{
  if (!f) {
    do { *b++ = 0; 
#if defined(EMSCRIPTEN)||defined(__APPLE__)||defined(REWAMP_SC68)
		if (gIntScopeBufs && gMixBuffer && gIntScopeBufs[3]) gIntScopeBufs[3][b - gMixBuffer] = 0;
#endif  	
	} while (--n);
    ...
    do {
	  ...
	  v = ( v << 16 ) | ( v & 0xFFFF );
#if defined(EMSCRIPTEN)||defined(__APPLE__)||defined(REWAMP_SC68)
		if (gIntScopeBufs && gMixBuffer && gIntScopeBufs[3]) gIntScopeBufs[3][b - gMixBuffer] = v;
#endif  	
      *b++ = v;
    } while (--n);
  }
...
static void mix_ste(mw_t * const mw, s32 *b, int n)
{
  ...
  int                vr = _VOL(right);
#if defined(EMSCRIPTEN)||defined(__APPLE__)||defined(REWAMP_SC68)
  /* rewamp: STE DMA is voice 3 on YM hardware -- muting it zeroes the DMA
   * contribution in the REAL mix (YM passthrough `ym` is untouched). */
  if (!rewamp_mw_active) vl = vr = 0;
#endif
  const int        loop = mw->map[MW_ACTI] & 2;
  const int        mono = (mw->map[MW_MODE]>>7) & 1;
  const uint_t      frq = 50066u >> ((mw->map[MW_MODE]&3)^3);
...
#if defined(EMSCRIPTEN)||defined(__APPLE__)||defined(REWAMP_SC68)
		if (gIntScopeBufs && gMixBuffer && gIntScopeBufs[3]) gIntScopeBufs[3][b - gMixBuffer] = *b;
#endif  	
	  b++;
