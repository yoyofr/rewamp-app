...
/* rewamp: direct active-channels setter (bit i set = voice i AUDIBLE) --
 * the option68 "amiga-chans" plumbing is overkill from a decoder plugin. */
void rewamp_paula_set_active(int mask) { pl_chans = 15 & mask; }
#endif
		    ...
		    s32 * b, int n)
{
  ...
  do {
    ...
    *b2 += v0;
    b2	+= 2;
#if defined(EMSCRIPTEN)||defined(__APPLE__)||defined(REWAMP_SC68)
    if (gIntScopeBufs && gIntScopeBufs[N]) gIntScopeBufs[N][i] = v0;
    i++;
#endif
