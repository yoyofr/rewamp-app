 ...
 */
static u8 trap_func[] = {
#if defined(EMSCRIPTEN)||defined(__APPLE__)||defined(REWAMP_SC68)
...
static void clear_scopes(sc68_t * sc68) {
	...
	/* rewamp: address THIS instance's scopes -- gIntScopeBufs is a
	 * process-global that may currently point at another instance. */
	for (int i= 0; i<n; i++) {	// only the used ones
		if (sc68->mix.scopes[i]) {
			memset(sc68->mix.scopes[i], 0, bufreq << 2);
		}
	}
}
...
/* rewamp: point the scope/mix process-globals at THIS instance. Must be
 * called before every sc68_process() that should produce scope data --
 * another sc68_t (the plugin's subsong-probe cache) may have been created
 * or destroyed in between, on another thread. */
void rewamp_sc68_bind_scopes(sc68_t * sc68) {
	if (!sc68) return;   /* is_sc68() is declared further down this TU */
	gIntScopeBufs = &(sc68->mix.scopes[0]);
	gMixBuffer    = (s32*)sc68->mix.buffer;
}
#endif
...
sc68_t * sc68_create(sc68_create_t * create)
{
  ...
  /*   sc68_unic = sc68; */
	...
	/* rewamp: do NOT bind gIntScopeBufs here. The scope plumbing uses two
	 * process-globals (gIntScopeBufs, gMixBuffer) and rewamp keeps a second,
	 * never-playing sc68_t alive to probe subsong counts -- binding at create
	 * made that probe instance (scopes all NULL) hijack the globals out from
	 * under the decoder rendering on the audio thread => NULL deref in
	 * ym_blep/paulaemul. The plugin calls rewamp_sc68_bind_scopes() on the
	 * instance it is about to render instead. */
...
#if defined(EMSCRIPTEN)||defined(__APPLE__)||defined(REWAMP_SC68)
	/* rewamp: only clear the globals when they point at THIS instance --
	 * destroying the probe instance used to zero the PLAYING one's
	 * gMixBuffer (paulaemul indexes `b - gMixBuffer`). */
	if (gMixBuffer == (s32*)sc68->mix.buffer) gMixBuffer = 0;
	if (gIntScopeBufs == &(sc68->mix.scopes[0])) gIntScopeBufs = 0;
	sc68->mix.buffer = 0;
	destroy_scopes(sc68);
#endif
...
/* rewamp: per-voice note state for the notation visualizer. The trace
 * streams carry waveform but no pitch, so read the chips' live registers.
 *
 *   freq  : Hz, 0 when the voice has no discrete pitch (PCM / digidrum).
 *   gate  : voice currently enabled (YM mixer/volume, Paula DMA).
 *   vol   : 0..255 note amplitude, where a register carries it honestly:
 *           Paula's per-voice volume (0..64) and the STE DMA on/off. The YM's
 *           volume register does NOT: SID-voice and digidrum tricks rewrite it
 *           at kHz rates, so sampled once per audio chunk it aliases into
 *           noise. `swing_amp` therefore asks the caller to measure the YM
 *           voices' amplitude from their trace instead (peak-to-peak over the
 *           chunk). Conversely a Paula trace is sampled PCM whose local peak
 *           wanders with the waveform, so its register is the honest source.
 *   sample: Paula only -- the voice's current sample start address. Together
 *           with `gate` it lets the caller spot a re-triggered sample that
 *           keeps the same volume (very common: the tracker just restarts the
 *           instrument), which the volume rule alone cannot see.
 *
 * YM amplitude is the chip's own 5-bit DAC level, so fixed-volume and
 * hardware-envelope voices are on one scale: fixed = ((v&15)<<1)|1 (the very
 * mapping ym_blep's volmask uses), envelope = the live envelope output.
 */
void rewamp_sc68_get_notes(sc68_t * sc68, rewamp_sc68_notes_t * out)
{
	int i, hw;
	memset(out, 0, sizeof(*out));
	if (!sc68 || !sc68->disk || sc68->track <= 0) return;
	hw = sc68_used_hw(sc68, sc68->track);

	if (hw & 4) {                             /* Amiga / Paula: voices 0-3 */
		paula_t * const pa = sc68->paula;
		unsigned clk;
		if (!pa) return;
		clk = (pa->clock == PAULA_CLOCK_NTSC) ? PAULA_NTSC_FRQ : PAULA_PAL_FRQ;
		for (i = 0; i < 4; i++) {
			const u8 * const r = pa->map + PAULA_VOICE(i);
			const unsigned per = ((unsigned)r[6] << 8) + r[7];
			unsigned v = r[9] & 127;
			const int on = ((pa->dmacon >> 9) & (pa->dmacon >> i)) & 1;
			if (v > 64) v = 64;                    /* Paula clamps above 64 */
			out->gate[i]   = on;
			out->sample[i] = (unsigned)(pa->voice[i].start >> pa->ct_fix);
			if (on && per && v) {
				out->freq[i] = (unsigned)(440.0 * (double)clk / (double)per / 8287.0);
				out->vol[i]  = v * 255u / 64u;
			}
		}
		out->count = 4;
		return;
	...
	{                                         /* Atari ST: YM 0-2 (+ STE 3) */
		ym_t * const ym = sc68->ym;
		unsigned clk, mixer, nper;
		if (!ym) return;
		clk   = (unsigned) ym->clock;
		mixer = ym->reg.name.ctl_mixer;
		nper  = ym->reg.name.per_noise & 31;
		for (i = 0; i < 3; i++) {
			const unsigned tper =
				((unsigned)ym->reg.index[i*2] |
				 ((unsigned)ym->reg.index[i*2+1] << 8)) & 0xFFF;
			const unsigned vreg = ym->reg.index[8+i];
			const int tone_on  = !((mixer >> i) & 1);
			const int noise_on = !((mixer >> (i+3)) & 1);
			const int use_env  = (vreg & 0x10) != 0;
			const int audible  = use_env || (vreg & 0x0F);
			const unsigned per = tone_on ? tper : (noise_on ? nper : 0);
			(void)use_env;

			out->gate[i] = audible != 0;
			if (!audible) continue;

			/* per == 0: tone AND noise mixed out while the volume register
			 * swings -- the classic ST volume-only digidrum. It sounds, with
			 * no discrete pitch (freq 0), like the STE DAC voice. */
			out->freq[i] = per ? clk / (16 * per) : 0;
		}
		out->count     = 3;
		out->swing_amp = 1;               /* YM: measure amplitude from trace */
		if (hw & 2) {                     /* STE MicroWire DMA = voice 3 */
			mw_t * const mw = sc68->mw;
			out->gate[3] = mw && (mw->map[MW_ACTI] & 1);
			out->vol[3]  = out->gate[3] ? 255 : 0;   /* PCM: no level to read */
			out->count   = 4;
		}
	}
}

/* rewamp: per-voice mute, generic_mute_mask convention (bit i set = voice i
 * MUTED). Voice layout matches chnTable/the scope streams: YM hardware =
 * voices 0-2 (YM A/B/C) + voice 3 (STE DMA); Amiga = voices 0-3 (Paula).
 * YM goes through ym_active_channels (honored by ym_puls natively and by
 * the rewamp dacstate patch in ym_blep); Paula/STE through the rewamp
 * setters patched into paulaemul.c/mwemul.c. */
void rewamp_sc68_set_mute(sc68_t * sc68, unsigned mask)
{
	extern void rewamp_paula_set_active(int mask);
	extern int rewamp_mw_active;
	int hw;
	if (!sc68 || !sc68->disk || sc68->track <= 0) return;
	hw = sc68_used_hw(sc68, sc68->track);
	if (hw & 4) {                 /* Amiga/Paula: voices 0-3 */
		rewamp_paula_set_active(~mask & 15);
	} else {                      /* Atari ST: YM 0-2 + STE DMA 3 */
		ym_active_channels(sc68->ym, mask & 7, ~mask & 7);
		rewamp_mw_active = !((mask >> 3) & 1);
	}
}
...
int sc68_cntl(sc68_t * sc68, int fct, ...)
{
  ...
  // REWAMP_SC68: dial68/ (the GUI dialog helpers) is not part of the vendored
  // file set, and rewamp never issues SC68_DIAL. Apple got away with it because
  // Mach-O dead-strips the unreachable case before the link; ld.lld with
  // --no-undefined does not, so the Android link failed on `dial68`.
  case SC68_DIAL: {
