...
void rewamp_sc68_set_mute(sc68_t * sc68, unsigned mask); /* per-voice mute */
void rewamp_sc68_bind_scopes(sc68_t * sc68); /* point scope globals at sc68 */
/* Per-voice note state for the notation visualizer (see api68.c). gate/sample
 * let the caller spot a re-triggered sample that keeps the same volume. */
typedef struct {
  unsigned freq[4];    /**< Hz, 0 = no discrete pitch (PCM/digidrum).   */
  unsigned vol[4];     /**< 0..255 note amplitude (unset when swing_amp). */
  unsigned gate[4];    /**< voice enabled (YM mixer/vol, Paula DMA).    */
  unsigned sample[4];  /**< Paula sample start address; 0 elsewhere.    */
  int      count;      /**< number of valid voices.                     */
  int      swing_amp;  /**< 1 = derive amplitude from the trace (YM).   */
} rewamp_sc68_notes_t;
