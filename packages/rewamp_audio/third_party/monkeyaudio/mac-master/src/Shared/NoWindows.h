...
// rewamp: les min/max ci-dessous sont des MACROS, et sous libstdc++ (Linux)
// <math.h> tire <cmath> -> bits/specfun.h -> bits/stl_algobase.h, où std::min
// et std::max sont DÉCLARÉS: la macro les rend impossibles à parser (des
// centaines d'erreurs dans les en-têtes système, jamais dans notre code).
// libc++ (Apple) n'a pas cette chaîne d'inclusion, d'où l'invisibilité côté
// macOS/iOS. On force donc les en-têtes qui déclarent min/max à être ANALYSÉS
// AVANT que les macros n'existent — leurs gardes d'inclusion neutralisent
// toute ré-inclusion ultérieure par All.h ou MACLib.
#ifdef __cplusplus
#include <algorithm>
#include <limits>
#include <cmath>
