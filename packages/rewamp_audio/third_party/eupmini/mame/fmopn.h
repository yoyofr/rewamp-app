 ...
 * rewamp: FM Towns EUPHONY uses ONLY the YM2612 path, so the OPN/OPNA/OPNB
 * emulators are hard-forced off. Upstream gated these on SNDDEV_YM2203/2608/
 * 2610, but on Apple those macros leak in POD-WIDE from libvgm (which vendors
 * the same-named chips), switching on BUILD_YM2608 etc. — code that references
 * a commented-out OPN.logger field and would not compile. Hard-coding them off
 * is build-system-independent (no reliance on per-file -U ordering) and can't
 * regress: this fmopn.c only ever drives YM2612. */
