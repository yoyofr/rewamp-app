...
extern int gbs_seek_needed;
/* Oscilloscope ring size (samples).  Must match the value the gbsplay plugin
 * passes to rewamp_channel_data_set_ring_write_size().  m_voice_current_ptr is
 * a fixed-point cursor (<<MODIZER_OSCILLO_OFFSET_FIXEDPOINT), same convention as
 * libnsfplay; ring_read() shifts it back down to a sample index. */
...
void gbhw_flush_buffer(struct gbhw *gbhw)
{
    ...
    for (int ii=0;ii<4;ii++) {
        ...
        for (i=0; i<gbhw->soundbuf->samples; i++) {
            ...
            int64_t idx = m_voice_current_ptr[ii] >> MODIZER_OSCILLO_OFFSET_FIXEDPOINT;
            m_voice_buff[ii][idx] =
                LIMIT8(((l_out * gbhw->master_volume / MASTER_VOL_MAX)>>7));
            m_voice_current_ptr[ii] += (1 << MODIZER_OSCILLO_OFFSET_FIXEDPOINT);
            if ((m_voice_current_ptr[ii] >> MODIZER_OSCILLO_OFFSET_FIXEDPOINT) >= GBS_OSCILLO_SIZE)
                m_voice_current_ptr[ii] -= ((int64_t)GBS_OSCILLO_SIZE << MODIZER_OSCILLO_OFFSET_FIXEDPOINT);
        }
