...
//        if( abs( j ) > vu[i] ) vu[i] = abs( j );
          ...
          // Ring size MUST match rewamp_plugin_hvl.cpp's set_ring_write_size —
          // see PLUGINS.md §2.3 (undersized *2=1024 silently disabled the
          // scope's stabilization trigger; fixed to *4*2=4096, 2026-07-07).
          if (!(generic_mute_mask&(1<<i))) m_voice_buff[i][(m_voice_current_ptr[i]>>MODIZER_OSCILLO_OFFSET_FIXEDPOINT)&(SOUND_BUFFER_SIZE_SAMPLE*4*2-1)]=LIMIT8(val >> 6);

          m_voice_current_ptr[i]+=1<<MODIZER_OSCILLO_OFFSET_FIXEDPOINT;
          if ((m_voice_current_ptr[i]>>MODIZER_OSCILLO_OFFSET_FIXEDPOINT)>SOUND_BUFFER_SIZE_SAMPLE*4*2) m_voice_current_ptr[i]-=(SOUND_BUFFER_SIZE_SAMPLE*4*2<<MODIZER_OSCILLO_OFFSET_FIXEDPOINT);
          //TODO:  MODIZER changes end / YOYOFR
      }
