...
int snsf_start(const unsigned char *lrombuf, INT32 lromsize, const unsigned char *srambuf, INT32 sramsize)
{
	...
	// rewamp: InitSnes9X re-inits the DSP with stereo_switch = every voice
	// audible, but dwChannelMuteOld is a file-static that survives a restart —
	// leaving it stale would make BUFFER::Fill() skip the S9xSetSoundControl
	// call and silently drop the mute mask after a backward seek. Reset the pair
	// to the state the hardware is actually in (all 8 voices on).
	dwChannelMute = dwChannelMuteOld = 0xFF;
