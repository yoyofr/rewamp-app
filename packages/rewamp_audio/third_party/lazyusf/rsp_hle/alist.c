...
//YOYOFR
#include <stdint.h>   // uint32_t — was <_types/_uint32_t.h> (Apple SDK-only, doesn't exist on Android/Linux)
...
// channels, no explicit "note off"). A slot that stops being resampled
// simply never gets called again — with nothing to reset it, ring_read()
// (rewamp_channel_data.c) keeps reading the SAME frozen window ending at its
// now-static m_voice_current_ptr, showing a static last-waveform snapshot
// forever instead of going silent.
//
// Fixed with an elapsed-time decay, NOT a "touched this read() call" flag:
// the RSP audio-list task (which is what actually calls alist_resample, per
// voice, below) doesn't run once per rewamp_plugin_lazyusf.cpp read() call —
// usf_render() only re-runs it once per real N64 "audio frame" and drains an
// internal buffer the rest of the time, so a per-read()-call touched flag
// was cleared far more often than voices actually went idle (blanked the
// whole scope). g_lazyusf_read_tick (defined in rewamp_plugin_lazyusf.cpp,
// incremented once per read() call) is the shared clock: alist_resample
// stamps a voice's last-touched tick wherever it actually writes, and
// lazyusf_clear_stale_voices() (same .cpp, called after each read() batch)
// clears a slot once enough ticks (~0.3s of real audio) have passed with no
// write — see the comment there for the exact threshold.
extern int64_t g_lazyusf_read_tick;
int64_t g_lazyusf_voice_last_touched_tick[32];
//YOYOFR
