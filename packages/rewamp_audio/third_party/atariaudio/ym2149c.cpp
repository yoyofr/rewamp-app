...
#include "ModizerVoicesData.h"   //YOYOFR: oscilloscope + mute framework
...
int16_t Ym2149c::ComputeNextSample(uint32_t* pSampleDebugInfo)
{
	...
	//YOYOFR: per-voice oscilloscope (voices 0..2) + generic_mute_mask.
	// s_ViewVolTab (ym2149_tables.h) is the library author's OWN lookup, built
	// specifically "for oscillators viewing display per voice" — it re-centers
	// the level around 0 via k15toS8's `^0x80` (unlike s_ym2149LogLevels, whose
	// raw gated-envelope amplitude is only ever positive and correlates poorly
	// with the scope's periodicity trigger). Use it verbatim for the ring;
	// levelA/B/C (already computed from s_ym2149LogLevels above) stay the audio
	// mix values untouched here.
	int8_t viewA = (int8_t)(uint8_t)s_ViewVolTab[indexA];
	int8_t viewB = (int8_t)(uint8_t)s_ViewVolTab[indexB];
	int8_t viewC = (int8_t)(uint8_t)s_ViewVolTab[indexC];
	// Muting zeroes the voice's contribution to the ACTUAL mix too (not just the
	// scope), matching every other adopted core's mute behaviour.
	if (generic_mute_mask & 1) { levelA = 0; viewA = 0; }
	if (generic_mute_mask & 2) { levelB = 0; viewB = 0; }
	if (generic_mute_mask & 4) { levelC = 0; viewC = 0; }
	// Ring mask MUST match rewamp_plugin_sndh.cpp's SNDH_RING_SIZE
	// (SOUND_BUFFER_SIZE_SAMPLE*4*2): the scope's trigger/stabilization search
	// needs outLen+TRIGGER_SEARCH_LEN (~2212) samples of ring history — a
	// smaller ring (bare SOUND_BUFFER_SIZE_SAMPLE, first tried here) makes the
	// search window come out empty and the oscilloscope never locks phase.
	if (m_voice_buff[0]) m_voice_buff[0][(m_voice_current_ptr[0]>>MODIZER_OSCILLO_OFFSET_FIXEDPOINT)&(SOUND_BUFFER_SIZE_SAMPLE*4*2-1)] = viewA;
	if (m_voice_buff[1]) m_voice_buff[1][(m_voice_current_ptr[1]>>MODIZER_OSCILLO_OFFSET_FIXEDPOINT)&(SOUND_BUFFER_SIZE_SAMPLE*4*2-1)] = viewB;
	if (m_voice_buff[2]) m_voice_buff[2][(m_voice_current_ptr[2]>>MODIZER_OSCILLO_OFFSET_FIXEDPOINT)&(SOUND_BUFFER_SIZE_SAMPLE*4*2-1)] = viewC;
	m_voice_current_ptr[0] += 1<<MODIZER_OSCILLO_OFFSET_FIXEDPOINT;
	m_voice_current_ptr[1] += 1<<MODIZER_OSCILLO_OFFSET_FIXEDPOINT;
	m_voice_current_ptr[2] += 1<<MODIZER_OSCILLO_OFFSET_FIXEDPOINT;
	// Notes: instantaneous tone/noise period → Hz, 0 when the voice is silent
	// (matches getYM2149_Freq's own formula so the note display and the
	// oscillo/freq probe never disagree).
	for (int v = 0; v < 3; v++) {
		double period = 0;
		if (!(m_toneMask & (1u<<(v*4))))        period = m_tonePeriod[v];
		else if (!(m_noiseMask & (1u<<(v*4))))  period = m_noisePeriod;
		vgm_last_note[v] = (period > 0) ? (unsigned int)(m_ymClockOneEighth / period / 2) : 0;
		vgm_last_vol[v]  = (v==0?levelA:(v==1?levelB:levelC)) > 0 ? 1 : 0;
	}
	//YOYOFR end
