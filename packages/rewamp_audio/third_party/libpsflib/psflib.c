...
static int psf_load_internal( psf_load_state * state, const char * file_name )
{
    ...
    /* rewamp: pour le panneau ⓘ. Cette fonction ouvre le PSF principal ET
       chaque `_lib` référencé (elle recurse), donc c'est le seul endroit qui
       connaisse la chaîne complète — et un `.psflib` porte le nom du JEU, pas
       celui du morceau: aucune heuristique de nom ne peut le retrouver. */
    if ( file ) rewamp_loaded_files_add( full_path );
