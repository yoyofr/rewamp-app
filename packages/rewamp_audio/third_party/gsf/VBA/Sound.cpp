...
void soundMix() {
    ...
    // rewamp/YOYOFR: honor generic_mute_mask on the AUDIO output, not just the
    // scope. Voices 0-3 = PSG soundBuffer[0..3], 4-5 = DirectSound
    // directBuffer[0..1]. Zero the muted voice's source sample for THIS frame so
    // it drops from both the mix (res) and the scope (sndChanL/R read below); the
    // channel keeps emulating (envelopes advance) so unmute is click-free.
    if (generic_mute_mask) {
        for (int v = 0; v < 4; v++)
            if ((generic_mute_mask >> v) & 1) soundBuffer[v][soundIndex] = 0;
        if ((generic_mute_mask >> 4) & 1) directBuffer[0][soundIndex] = 0;
        if ((generic_mute_mask >> 5) & 1) directBuffer[1][soundIndex] = 0;
    }
  ...
  if(soundReverse)
    ...
    for (int jj=0;jj<6;jj++) {
        ...
        if ((m_voice_current_ptr[jj]>>MODIZER_OSCILLO_OFFSET_FIXEDPOINT)>=SOUND_BUFFER_SIZE_SAMPLE*4*2) m_voice_current_ptr[jj]-=(SOUND_BUFFER_SIZE_SAMPLE*4*2)<<MODIZER_OSCILLO_OFFSET_FIXEDPOINT;   // ring size must match rewamp_plugin_gsf.cpp — PLUGINS.md §2.3
    }
    //TODO:  MODIZER changes end / YOYOFR
}
#else
