...
int64_t fmpmini_getLength(void) {
    ...
    // rewamp: init to -1 (was 0) so ppz8.c's `if (fmp_ppz8_maxChannel<p)` still
    // records channel 0, AND "a .pvi/.ppz is referenced but no PPZ8 channel ever
    // plays" stays distinguishable from "channel 0 plays" — see channelsNb's +1.
    fmp_ppz8_maxChannel=-1;
...
int fmpmini_channelsNb(void){
    ...
    // rewamp: +1 because fmp_ppz8_maxChannel is the highest PLAYING channel
    // INDEX (0-based), so the voice count is that index + 1. Without it the plugin
    // allocated one voice ring too few and ppz8.c's m_voice_buff[16+maxChannel]
    // write NULL-deref'd (SEGV on any .ozi/.opi/.ovi that uses PPZ8). With the
    // -1 init above, "no PPZ8 plays" gives 16 + (-1) + 1 = 16 (no extra voice).
    if (g.fmfile->driver.fmp.pvi_name[0] || g.fmfile->driver.fmp.ppz_name[0]) return 16+fmp_ppz8_maxChannel+1;
    else return 16;
}
