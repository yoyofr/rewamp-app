...
int SID::clock(unsigned int cycles, short* buf)
{
    ...
    while (cycles > 0)
    {
        ...
        for (int j=0;j<4;j++) {
            m_voice_buff[sid_idx+0][(m_voice_current_ptr[sid_idx+0]>>MODIZER_OSCILLO_OFFSET_FIXEDPOINT)&(SOUND_BUFFER_SIZE_SAMPLE*8-1)]=LIMIT8((int)(sid_v1));
            m_voice_buff[sid_idx+1][(m_voice_current_ptr[sid_idx+1]>>MODIZER_OSCILLO_OFFSET_FIXEDPOINT)&(SOUND_BUFFER_SIZE_SAMPLE*8-1)]=LIMIT8((int)(sid_v2));
            m_voice_buff[sid_idx+2][(m_voice_current_ptr[sid_idx+2]>>MODIZER_OSCILLO_OFFSET_FIXEDPOINT)&(SOUND_BUFFER_SIZE_SAMPLE*8-1)]=LIMIT8((int)(sid_v3));
            m_voice_buff[sid_idx+3][(m_voice_current_ptr[sid_idx+3]>>MODIZER_OSCILLO_OFFSET_FIXEDPOINT)&(SOUND_BUFFER_SIZE_SAMPLE*8-1)]=LIMIT8((sid_v4>>10));
