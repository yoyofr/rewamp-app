...
//TODO: MODIZER changes / YOYOFR — per-voice oscilloscope + mute capture.
#ifdef REWAMP_WITH_VIO2SF
extern "C" {
#include "ModizerVoicesData.h"   /* m_voice_buff, m_voice_current_ptr, generic_mute_mask, LIMIT8 */
#include "ModizerConstants.h"    /* SOUND_BUFFER_SIZE_SAMPLE, MODIZER_OSCILLO_OFFSET_FIXEDPOINT */
}
/* Per-output-sample per-channel scope write.  chval[] holds each channel's
 * post-volume contribution; muted channels are zeroed in place so the caller
 * silences them in the mix too. Ring mask matches the plugin's
 * set_ring_write_size(SOUND_BUFFER_SIZE_SAMPLE*2*4). */
static inline void rewamp_vio2sf_capture(melonDS::s32* chval)
{
    const int64_t RING = SOUND_BUFFER_SIZE_SAMPLE * 2 * 4;
    for (int c = 0; c < 16; c++) {
        if ((generic_mute_mask >> c) & 1) chval[c] = 0;   /* mute audio */
        if (!m_voice_buff[c]) continue;
        /* chval is CurSample(±2^15) << VolumeShift(0..4) * Volume(0..128), so its
         * max magnitude is ~2^26; even a normal full-volume channel reaches ~2^22.
         * >>15 put a full-volume channel right at the LIMIT8 clip edge (louder
         * ones saturated the scope); >>17/18 still brushed it on loud rips.
         * >>19 = full-volume ≈ quarter scale, no visible clipping. */
        int outp = (int)(chval[c] >> 19);                 /* post-vol → int8 scope shape */
        int64_t ofs = m_voice_current_ptr[c];
        m_voice_buff[c][(ofs >> MODIZER_OSCILLO_OFFSET_FIXEDPOINT) & (RING - 1)] = LIMIT8(outp);
        ofs += (int64_t)1 << MODIZER_OSCILLO_OFFSET_FIXEDPOINT;
        while ((ofs >> MODIZER_OSCILLO_OFFSET_FIXEDPOINT) >= RING)
            ofs -= RING << MODIZER_OSCILLO_OFFSET_FIXEDPOINT;
        m_voice_current_ptr[c] = ofs;
    }
}
#endif
...
void SPU::Mix(u32 spucycles)
{
    ...
    {
        s32 chval[16];
        for (int i = 0; i < 16; i++) chval[i] = Channels[i].DoRun(spucycles);

#ifdef REWAMP_WITH_VIO2SF
        rewamp_vio2sf_capture(chval);   // per-voice scope + mute (zeroes muted chval)
#endif
        s32 ch0 = chval[0], ch1 = chval[1], ch2 = chval[2], ch3 = chval[3];

        // TODO: addition from capture registers
        Channels[0].PanOutput(chval[0], left, right);
        Channels[2].PanOutput(chval[2], left, right);

        if (!(Cnt & (1<<12))) Channels[1].PanOutput(chval[1], left, right);
        if (!(Cnt & (1<<13))) Channels[3].PanOutput(chval[3], left, right);

        for (int i = 4; i < 16; i++)
        {
            Channels[i].PanOutput(chval[i], left, right);
        }
