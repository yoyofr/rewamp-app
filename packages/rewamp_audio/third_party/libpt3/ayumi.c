...
static void update_mixer(struct ayumi* ay,int ch) {
  ...
  for (i = 0; i < TONE_CHANNELS; i += 1) {
    ...
    ay->right += ay->dac_table[out] * ay->channels[i].pan_right;
      ...
      // YOYOFR (rewamp): ring mask was missing entirely and the wraparound
      // (below, in ayumi_process) reset at bare SOUND_BUFFER_SIZE_SAMPLE
      // (512) instead of *4*2 (4096) -- same undersized-ring class as the
      // SNDH/HVL/V2M/GSF sweep (PLUGINS.md §2.3): the scope's trigger
      // stabilization search needs ~2212 samples of ring history, so it
      // silently never ran. Added the explicit mask defensively too.
      m_voice_currentChannel=ch*3+i;
      m_voice_buff[m_voice_currentChannel][(m_voice_current_ptr[m_voice_currentChannel]>>MODIZER_OSCILLO_OFFSET_FIXEDPOINT)&(SOUND_BUFFER_SIZE_SAMPLE*4*2-1)]=LIMIT8( (ay->dac_table[out]*100) );
  }
}
