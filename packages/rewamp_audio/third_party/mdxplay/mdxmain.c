...
if (!self_construct()) {
    ...
    *mdx = mdx_open_mdx( filename );
    /* rewamp: upstream calls error_end() here, which exit(1)s — that would take
       the whole app down on an unreadable/short file. Report the failure to the
       caller instead. */
    if ( (*mdx) == NULL ) { self_destroy(); return -1; }
...
_load_pdx_data(char* name, long* out_length) 
{
  ...
  *(p+1) = '\0';
  ...
  /* rewamp: pour le panneau ⓘ. Un .mdx a besoin de son .pdx (la banque
     d'échantillons PCM8), et c'est ici qu'il est enfin ouvert — après une
     recherche insensible à la casse dans le dossier, donc le nom réel n'est
     connu que maintenant. */
  rewamp_loaded_files_add( p2 );
