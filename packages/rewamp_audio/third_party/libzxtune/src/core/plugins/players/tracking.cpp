...
namespace Module
{
  ...
  private:
    ...
    void SetPattern(uint_t pat)
    {
      ...
      //REWAMP: SparsedObjectsStorage::Get returns a NULL Ptr for an index it does
      //not hold, and SetLine dereferences it right away — a hard assert (and, in a
      //release build, a null deref) on any module whose order list names a pattern
      //the file never defines. That is reachable from ordinary content: a Vortex
      //text module whose body parse stops early (ParseBody bails at the first line
      //that is not a section header — a blank line between sections does it) keeps
      //its full PlayOrder while losing the patterns past the cut. An undefined
      //pattern plays as nothing, which is what the zero-length StubPattern already
      //means for an out-of-range POSITION; reuse it for a missing PATTERN so the
      //order entry is skipped instead of taking the process down.
      if (!CurPatternObject)
      {
        CurPatternObject = StubPattern::Create();
      }
  ...
  //YOYOFR (rewamp): every track-based player builds its data iterator from its
  //TrackModel through this one function — eighteen of them, from PT3 to the
  //Vortex text format — so collecting the model HERE gives the pattern
  //visualizer the whole song structure without touching a single player. A
  //TurboSound pair creates two iterators, hence a list rather than one slot;
  //the collection is cleared by the wrapper before the renderer is built.
  namespace
  {
    std::vector<TrackModel::Ptr> RewampModels;
  }

  void RewampTrackModelsClear()
  {
    RewampModels.clear();
  }

  std::size_t RewampTrackModelsCount()
  {
    return RewampModels.size();
  }

  TrackModel::Ptr RewampTrackModelAt(std::size_t idx)
  {
    return idx < RewampModels.size() ? RewampModels[idx] : TrackModel::Ptr();
  }

  TrackStateIterator::Ptr CreateTrackStateIterator(TrackModel::Ptr model)
  {
    RewampModels.push_back(model);
