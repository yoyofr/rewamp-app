...
namespace Module
{
  ...
  public:
    ...
    /* rewamp: les trois accesseurs MODIZER de notre Renderer (absents de
     * l'amont 738b71fc9b) — voies materielles CONCATENEES dans l'ordre des
     * delegues, comme MultiTrackState::Channels() les additionne. */
    virtual int GetHWChannels()
    {
      int res = 0;
      for (std::size_t idx = 0, lim = Delegates.size(); idx != lim; ++idx)
      {
        res += Delegates[idx]->GetHWChannels();
      }
      return res;
    }
    ...
    virtual const char *GetHWChannelName(int chan)
    {
      for (std::size_t idx = 0, lim = Delegates.size(); idx != lim; ++idx)
      {
        const int n = Delegates[idx]->GetHWChannels();
        if (chan < n)
        {
          return Delegates[idx]->GetHWChannelName(chan);
        }
        chan -= n;
      }
      return Delegates.front()->GetHWChannelName(0);
    }

    virtual const char *GetHWSystemName()
    {
      return Delegates.front()->GetHWSystemName();
    }
