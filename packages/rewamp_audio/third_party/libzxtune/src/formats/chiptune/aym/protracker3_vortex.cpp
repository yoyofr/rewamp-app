  ...
  namespace VortexTracker2
  {
    ...
    */
    ...
    public:
      ...
      //REWAMP: the character this flag writes when the bit is SET. Vortex writers
      //do not all spell a set bit the same way (see ParseMasks), and the alternative
      //has to be mapped back onto this one rather than duplicated as a literal.
      static char SetChar()
      {
        return True;
      }
    ...
    */
    ...
    private:
      ...
      private:
        ...
        //REWAMP: these three are MASK bits, and the letter case is what carries them
        //— the format comment above spells it out: "t<true> T<false>", so the
        //LOWERCASE letter is the SET mask (generator silenced) and the uppercase one
        //is the clear mask (generator audible). VT II writes "Tne" = tone audible,
        //noise and envelope masked off; Ivan Pirog's .vt2 variant writes the set
        //mask as a dot instead ("T.."). Rejecting the dot fails the whole sample
        //section, hence the whole module — the file then reaches the player with no
        //patterns at all. Map the dot onto the SET character: mapping it the other
        //way round un-masks the noise and envelope generators on every sample line,
        //which is heard as a constant hiss over the music.
        static char MaskChar(char c, char set)
        {
          return c == '.' ? set : c;
        }
        ...
        {
          Require(str.size() == 3);
          ToneMask = ToneFlag(MaskChar(str[0], ToneFlag::SetChar())).AsBool();
          NoiseMask = NoiseFlag(MaskChar(str[1], NoiseFlag::SetChar())).AsBool();
          EnvMask = EnvelopeFlag(MaskChar(str[2], EnvelopeFlag::SetChar())).AsBool();
        }
    ...
    public:
      ...
      void Parse(PatternBuilder& patBuilder, Builder& builder) const
      {
        //REWAMP: the noise base is a per-LINE column here, but the receiving builder
        //stores it as a COMMAND in the current channel's cell (pt3_supp's
        //SetNoiseBase does Patterns.GetChannel().AddCommand). Emitting it before the
        //first StartChannel dereferences a null channel — a hard crash on any Vortex
        //text module with a non-zero noise column, which the binary PT3 path never
        //hits because there the command is parsed inside a channel. Which channel
        //carries it does not matter for playback: vortex_base.cpp puts NOISEBASE in
        //the global CommState, not in the channel's state. Attach it to channel 0.
        const uint_t noiseBase = Noise.AsInt();
        const uint_t envBase = Envelope.AsInt();
        for (uint_t idx = 0; idx != 3; ++idx)
        {
          builder.StartChannel(idx);
          if (0 == idx && 0 != noiseBase)
          {
    ...
    public:
      ...
      {
        Dbg("Parse body");
        //REWAMP: a blank line is a SEPARATOR, not the end of the body. Each section
        //object already consumes the empty line that terminates it, so a writer that
        //also puts a blank line BETWEEN sections leaves one the body loop sees first
        //— and treating that as end-of-body silently yields a module with zero
        //patterns, whose PlayOrder then names patterns that do not exist (a null
        //Pattern::Ptr, and an assert deep in TrackStateCursor). Real files do it:
        //Ivan Pirog's .vt2 variant, which VT II 1.0 beta 20 added a loader for,
        //writes "Colors=…\r\n" + terminator + one more blank line before
        //[Ornament1]. Skip blank runs and remember the end of the last section, so
        //the container limit stays where it was for files without them.
        std::size_t lastSectionEnd = Source.GetPosition();
        for (;;)
        {
          ...
          {
            //Not a section: the module ends here (a second [Module] in a
            //TurboSound pair lands on this branch).
            return lastSectionEnd;
          }
          lastSectionEnd = Source.GetPosition();
        }
      }
    private:
