...
// Android's bionic <sys/types.h> already defines `uint_t` (unsigned int, 32-bit),
// which clashes with boost::uint_fast32_t (64-bit) here. zxtune only requires
// uint_t to be >= 32-bit unsigned, so on Android reuse bionic's. (Apple/glibc do
// not define uint_t, so the boost typedef stands there.)
#ifndef __ANDROID__
