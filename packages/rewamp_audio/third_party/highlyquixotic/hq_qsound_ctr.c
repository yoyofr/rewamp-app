...
void qsoundc_update(void* param, UINT32 samples, INT16* output)
{
	...
	if (output)
	{
		...
		for (curSmpl = 0; curSmpl < samples; curSmpl++)
		{
            ...
            /* YOYOFR (rewamp): original Modizer app gate was HC_voicesMuteMask1,
             * an app-level global that doesn't exist in rewamp (always reads as
             * uninitialized 0 here -> always false -> scope permanently flatlined).
             * chip->voice_output[] is already correctly zeroed for muted voices by
             * chip->muteMask (see qsoundc_set_mute_mask, wired from
             * generic_mute_mask in the plugin) upstream in pcm_update/adpcm update
             * -- no extra gate needed here, just write the real value. */
            for (;;) {
                for (int jj=0;jj<16+3;jj++)
                    m_voice_buff[jj][(ofs_start>>MODIZER_OSCILLO_OFFSET_FIXEDPOINT)&(SOUND_BUFFER_SIZE_SAMPLE*4*2-1)]=LIMIT8((chip->voice_output[jj]>>4)*0.4f);
...
INLINE INT16 get_sample(struct qsound_chip *chip, UINT16 bank,UINT16 address)
{
	...
	// YOYOFR (rewamp): rom_addr from bank<<16|address can address up to 2GB,
	// but chip->romData is only chip->romSize bytes (the actual loaded SMP
	// section). This was a real heap-buffer-overflow read (confirmed via
	// ASan: ".../mshvsf0A.miniqsf" requests bank>=0x80, past an 8MB ROM) --
	// fixes a crash. Tried masking with chip->romMask first (wraps/mirrors,
	// like an underpopulated real ROM's unconnected high address lines), but
	// that aliases the out-of-range read onto unrelated in-range sample data
	// and audibly sounds like noise, not silence. Returning silence for the
	// genuinely out-of-bounds case is the safer choice: no crash, no noise,
	// and it doesn't invent sample data that isn't actually loaded.
...
static void state_normal_update(struct qsound_chip *chip)
{
	...
	for(ch=0; ch<2; ch++)
	{
		...
		for(v=0; v<19; v++)
		{
            ...
            // YOYOFR (rewamp): dropped the HC_voicesMuteMask1 gate (Modizer
            // app-level global, doesn't exist here -> always false -> QSound
            // was silent on every voice, not just the scope). chip->voice_output[v]
            // is already correctly zeroed for muted voices by chip->muteMask
            // (qsoundc_set_mute_mask, wired from generic_mute_mask in the
            // plugin) upstream -- no separate gate needed, same fix as the
            // scope write above.
