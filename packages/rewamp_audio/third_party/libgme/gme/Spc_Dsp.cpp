...
/* Échantillons consécutifs à sortie nulle, par voix — voir la capture de note.
 * ~8 ms à 32 kHz: assez long pour ignorer un passage par zéro, assez court
 * pour qu'une voix qui se tait le montre tout de suite. */
#define SPC_QUIET_SAMPLES 256
static int voice_quiet[8];
...
void Spc_Dsp::run( int clock_count )
{
	...
	do
	{
		...
		do
		{
			...
			{
				int output = 0;
				/* YOYOFR: déclaré ICI, pas dans le `if (env)` — la capture du
				 * scope et de la note doit tourner AUSSI quand la voix est
				 * éteinte, sinon elle reste figée sur son dernier son. */
				int newamp = 0;
                ...
                //TODO:  MODIZER changes start / YOYOFR
                /* HORS du `if (env)`, et c'est tout le correctif.
                 *
                 * La capture vivait à l'intérieur, avec le calcul de sortie.
                 * Or `env == 0` est EXACTEMENT l'état d'une voix éteinte: le
                 * bloc entier était sauté, donc plus rien n'était écrit — ni
                 * l'échantillon du scope, ni la note. Les deux restaient figés
                 * sur leur dernière valeur, pour toujours: une note tenue par
                 * une voix muette, et un oscilloscope qui continue d'afficher
                 * la forme d'onde d'un son terminé. Modizer a la même
                 * structure; ses 16 tranches tournantes de `vgm_last_*` le
                 * masquaient en partie, notre modèle par voix ne le masque
                 * plus du tout.
                 *
                 * `newamp` vaut 0 quand la voix est éteinte, et le scope veut
                 * précisément ce zéro. */
                const int scopeAmp = LIMIT8((newamp>>21)*3/4);
                {
                    const int p = (int)(m_voice_current_ptr[current_voice]>>MODIZER_OSCILLO_OFFSET_FIXEDPOINT);
                    m_voice_buff[current_voice][p    &(SOUND_BUFFER_SIZE_SAMPLE*4*4-1)]=scopeAmp;
                    m_voice_buff[current_voice][(p+1)&(SOUND_BUFFER_SIZE_SAMPLE*4*4-1)]=scopeAmp;
                }

                /* La NOTE ne se déduit NI de l'amplitude instantanée, NI de
                 * la seule enveloppe.
                 *
                 * Pas de l'amplitude: une forme d'onde traverse zéro deux fois
                 * par période, donc `if (newamp)` éteignait la note à chaque
                 * passage par zéro — sans conséquence la plupart du temps, mais
                 * l'instantané des notes est pris en bout de bloc et pouvait
                 * tomber précisément là.
                 *
                 * Pas de la seule enveloppe non plus: une voix reste en sustain
                 * sur une boucle SILENCIEUSE, enveloppe grande ouverte et sortie
                 * nulle. C'est l'autre moitié des « notes tenues sans son », et
                 * sortir la capture du `if (env)` n'y suffisait pas.
                 *
                 * Critère retenu: la voix n'est pas coupée (enveloppe, mute
                 * global flg bit 6, ses deux volumes) ET elle a écrit un
                 * échantillon de scope NON NUL récemment. Se caler sur
                 * `scopeAmp` plutôt que sur `newamp` brut donne la seule règle
                 * cohérente pour l'utilisateur — pas de note là où le scope ne
                 * montre rien — et un seuil franc (~-48 dBFS) au lieu du bit de
                 * poids faible, qui survit à des niveaux inaudibles. Le
                 * compteur absorbe les passages par zéro (isolés, même à 30 Hz)
                 * et coupe en ~8 ms.
                 *
                 * Mesuré (oracle par voix isolée, cf. scripts/verify_spc_voice_capture.sh):
                 * notes fantômes 81 -> 4 et 90 -> 2 sur deux SPC de Zelda 3. */
                if (scopeAmp) voice_quiet[current_voice] = 0;
                else if (voice_quiet[current_voice] < SPC_QUIET_SAMPLES)
                    voice_quiet[current_voice]++;
                {
                    const int audible = env && !(REG(flg) & 0x40)
                                     && (v->volume[0] || v->volume[1])
                                     && voice_quiet[current_voice] < SPC_QUIET_SAMPLES;
                    if (audible) {
                        int freq=((long long)(pitch)*440/(1<<12)); //assume ref is A4
                        vgm_last_note[current_voice]=freq;
                        vgm_last_vol[current_voice]=env>>3;
...
void Spc_Dsp::init( void* ram_64k )
{
	...
	memset( voice_quiet, 0, sizeof voice_quiet );   //YOYOFR
	mute_voices( 0 );
