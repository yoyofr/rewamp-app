...
//YOYOFR: capture par partie (voir src/rewamp_mt32_capture.h)
#include "../../../src/rewamp_mt32_capture.h"
#include "Synth.h"
#include "Tables.h"
#include "TVA.h"
#include "TVF.h"
...
void Partial::produceAndMixSample(IntSample *&leftBuf, IntSample *&rightBuf, LA32IntPartialPair *la32IntPair) {
	...
	//YOYOFR: contribution de CE partiel (avant mixage), normalisée comme le rendu flottant (/8192 ≈ pleine échelle)
	rewamp_mt32_run_add(poly->getPart()->getPartNum(), sampleNum, float(((sample * leftPanValue) >> 13) + ((sample * rightPanValue) >> 13)) * (1.0f / 8192.0f));
...
void Partial::produceAndMixSample(FloatSample *&leftBuf, FloatSample *&rightBuf, LA32FloatPartialPair *la32FloatPair) {
	...
	//YOYOFR: contribution de CE partiel (avant mixage)
	rewamp_mt32_run_add(poly->getPart()->getPartNum(), sampleNum, leftOut + rightOut);
	*(leftBuf++) += leftOut;
	*(rightBuf++) += rightOut;
}
