...
//REWAMP: NULL-terminate. sconfig_load()/others iterate `while(g_app_config[pn])`
//and check `== NULL`, but upstream ends the array with the STRING "0" (non-NULL)
//— so after "0" the loop reads g_app_config[3] out of bounds. The SDK's own
//.dylib gets away with it (adjacent data happens to be 0); linked into rewamp's
//big static pod the OOB pointer is garbage → strlen SIGSEGV at sv_init. Adding a
//real NULL terminator keeps every probed path and stops the over-read.
const char* g_app_config[] = { "1:/sunvox_dll_config.ini", "2:/sunvox_dll_config.ini", "0", (const char*)0 };
...
//REWAMP: per-module mute. SunVox honors PSYNTH_FLAG_MUTE internally (psynth_render)
//but exposes no public setter — the only mute path is the GUI. This surfaces the
//existing flag so the plugin can drive it from generic_mute_mask. A flag (not a ctl)
//is never overwritten by the tune's own pattern automation, unlike ctl-Volume.
extern "C" int rewamp_sv_set_module_mute( int slot, int mod_num, int mute )
{
    if( check_slot( slot ) ) return -1;
    psynth_module* m = psynth_get_module( mod_num, g_sv[ slot ]->net );
    if( !m ) return -1;
    if( mute ) m->flags |= PSYNTH_FLAG_MUTE;
    else       m->flags &= ~PSYNTH_FLAG_MUTE;
    return 0;
}
